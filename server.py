...
def _sylva_safe_filename(text, allow_dots=True):
    """Dosya adını güvenli hale getirirken Unicode harf/rakamları korur.
    Böylece dışa aktarma dosya adları aktif arayüz dilindeki analiz başlığını
    (Korece, Arapça, Çince, Kiril vb.) kaybetmez."""
    cleaned = str(text or '').replace('\x00', '')
    pattern = r'[^\w\-\.]+' if allow_dots else r'[^\w\-]+'
    cleaned = re.sub(pattern, '_', cleaned, flags=re.UNICODE)
...
def _build_lulc_symbology_zip(tif_bytes, index_name, safe_name, legend_labels=None, nodata_value=None):
    ...
    # kod -> (isim, (r,g,b)); etiket, istemcinin seçtiği dilde gönderilmişse
    # onu kullan. Böylece indirilen ArcMap lejantı ekrandaki dille birebir eşleşir.
    _label_override = {}
    ...
    # Böylece ArcMap lejantı tüm CORINE/ESA/MODIS sözlüğünü değil, ekrandaki AOI
    # sınıflarını gösterir. Piksel değerleri değişmez.
...
def _classify_aspect_breaks(band, valid_mask, breaks):
    """Aspect için kullanıcı sınıflarını kullanırken Kuzey sınıfının
    327.5–360° bölümünü kaybetme. Standart 9 sınıflı Aspect tablosunda 2. sınıf
    Kuzeydir ve son sınıf 327.5° civarında biter; bu bölüm aynı 2. sınıfa bağlanır.
    Kullanıcı Aspect sınıflarını gerçekten değiştirirse normal _classify_by_breaks
    davranışı korunur."""
    import numpy as np
    if not isinstance(breaks, list) or len(breaks) != 9:
        return _classify_by_breaks(band, valid_mask, breaks)
    try:
        second_min = float(breaks[1].get('min'))
        second_max = float(breaks[1].get('max'))
        last_max = float(breaks[-1].get('max'))
    except (TypeError, ValueError, AttributeError):
        return _classify_by_breaks(band, valid_mask, breaks)
    if not (np.isfinite(second_min) and np.isfinite(second_max) and np.isfinite(last_max)):
        return _classify_by_breaks(band, valid_mask, breaks)
    # Yalnızca ArcMap uyumlu standart Kuzey aralığı için özel sarma.
    if second_min > 0.5 or second_max > 30 or last_max < 300:
        return _classify_by_breaks(band, valid_mask, breaks)
    idx, code_info = _classify_by_breaks(band, valid_mask, breaks)
    north_wrap = valid_mask & (band >= last_max) & (band <= 360.000001)
    idx[north_wrap] = 2
    # 2. sınıfın etiketi/rengi kullanıcı tarafından gönderilen değerlerden aynen alınır.
    return idx.astype(np.uint8), code_info
...
def _enrich_vector_features(features, class_meta):
    ...
    for feat in features or []:
        ...
        # Custom classBreaks are encoded by reduceToVectors() as 1..N. Keep the
        # exact legend name/color even when GEE serializes class_value as a
        # numeric string/float; the fallback below also protects older payloads
        # that returned a zero-based class code.
...
@app.route('/api/carbon-simulation', methods=['POST'])
