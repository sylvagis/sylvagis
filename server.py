...
def _build_symbology_files_from_classes(byte_band, profile, code_info, safe_name, embed_colormap=True):
    ...
    with MemoryFile() as out_memfile:
        with out_memfile.open(**new_profile) as dst:
            ...
            # ArcMap'in uzak zoomlarda da aynı sınıflandırılmış rasterı
            # göstermesi için sınıflandırılmış TIFF'in kendi piramitlerini
            # oluştur. Sınıf kodları değişmez; yalnızca görüntüleme için
            # nearest-neighbour overview kullanılır.
            try:
                from rasterio.enums import Resampling
                _w, _h = int(new_profile.get('width', 0)), int(new_profile.get('height', 0))
                _levels = [2, 4, 8, 16, 32, 64] if min(_w, _h) >= 64 else [2, 4, 8, 16]
                _levels = [lv for lv in _levels if _w // lv >= 32 and _h // lv >= 32]
                if _levels:
                    dst.build_overviews(_levels, Resampling.nearest)
                    dst.update_tags(ns='rio_overview', resampling='nearest')
            except Exception as _ov_err:
                print('[SylvaGIS] ⚠️ Sınıflandırılmış raster overview üretilemedi:', _ov_err)
    ...
    # ArcMap'in RAT/VAT'i otomatik bağlamadığı sürümlerde bile ZIP içinde
    # doğrudan okunabilir, UTF-8 ve yalnızca GERÇEK kullanılan sınıfları içeren
    # bir lejant tablosu bulunur. Bu özellikle Aspect/TWI gibi topografik
    # sınıflandırmalarda önceki dildeki isimlerin korunması için önemlidir.
    _legend_lines = ['VALUE,CLASS_NAME,RED,GREEN,BLUE']
    for _code in sorted(code_info.keys()):
        _label, _rgb = code_info[_code]
        _legend_lines.append('{0},"{1}",{2},{3},{4}'.format(
            _code, str(_label).replace('"', '""'), _rgb[0], _rgb[1], _rgb[2]))
    legend_csv_bytes = ('\ufeff' + '\n'.join(_legend_lines) + '\n').encode('utf-8')
    ...
    return {
        ...
        '{}.legend.csv'.format(safe_name): legend_csv_bytes,
    }



# 30-language raster legend labels; synchronized with the web UI.
_RASTER_CLASS_LABELS_30={"en":{"lulc_tree":"Tree Cover / Forest","lulc_shrubland":"Shrubland","lulc_grassland":"Grassland / Pasture","lulc_cropland":"Cropland","lulc_urban":"Built-up / Urban","lulc_sparse":"Bare / Sparse Vegetation","lulc_snow":"Snow / Ice","lulc_water":"Water Body","lulc_wetland":"Wetland / Marsh","lulc_mangrove":"Mangrove","lulc_moss":"Moss / Lichen","modis_evergreen_needle":"Evergreen Needleleaf Forest","modis_evergreen_broad":"Evergreen Broadleaf Forest","modis_deciduous_needle":"Deciduous Needleleaf Forest","modis_deciduous_broad":"Deciduous Broadleaf Forest","modis_mixed_forest":"Mixed Forests","modis_closed_shrub":"Closed Shrublands","modis_open_shrub":"Open Shrublands","modis_woody_savanna":"Woody Savannas","modis_savanna":"Savannas","modis_grassland":"Grasslands","modis_wetland":"Permanent Wetlands","modis_cropland":"Croplands","modis_urban":"Urban and Built-up","modis_crop_natural":"Cropland / Natural Vegetation Mosaic","modis_snow":"Snow and Ice","modis_bare":"Barren or Sparsely Vegetated","modis_water":"Water Bodies","clc_urban_cont":"Continuous Urban Fabric","clc_urban_disc":"Discontinuous Urban Fabric","clc_industrial":"Industrial / Commercial","clc_road_rail":"Road / Rail Networks","clc_port":"Port Areas","clc_airport":"Airports","clc_mine":"Mineral Extraction Sites","clc_dump":"Dump Sites","clc_construction":"Construction Sites","clc_urban_green":"Green Urban Areas","clc_sport":"Sport / Leisure","clc_arable_dry":"Non-irrigated Arable Land","clc_arable_irr":"Irrigated Arable Land","clc_rice":"Rice Fields","clc_vineyard":"Vineyards","clc_fruit":"Fruit Trees","clc_olive":"Olive Groves","clc_pasture":"Pastures","clc_annual_mosaic":"Annual / Permanent Crops Mosaic","clc_complex_farm":"Complex Cultivation Patterns","clc_agri_natural":"Agriculture with Natural Vegetation","clc_agri_forest":"Agro-Forestry Areas","clc_broadleaf":"Broad-leaved Forest","clc_conifer":"Coniferous Forest","clc_mixed_forest":"Mixed Forest","clc_natural_grass":"Natural Grassland","clc_moor":"Moors / Heathland","clc_sclerophyllous":"Sclerophyllous Vegetation","clc_transitional":"Transitional Woodland-Shrub","clc_beach":"Beaches / Dunes / Sand","clc_bare_rock":"Bare Rock","clc_sparse_veg":"Sparsely Vegetated Areas","clc_burnt":"Burnt Areas","clc_glacier":"Glaciers / Perpetual Snow","clc_inland_marsh":"Inland Marshes","clc_peatbog":"Peat Bogs","clc_salt_marsh":"Salt Marshes","clc_salines":"Salines","clc_intertidal":"Intertidal Flats","clc_river":"Water Courses","clc_lake":"Water Bodies","clc_lagoon":"Coastal Lagoons","clc_estuary":"Estuaries","clc_sea":"Sea / Ocean"},"uz":{"lulc_tree":"Daraxt qoplami / Oʻrmon","lulc_shrubland":"Butazor","lulc_grassland":"Oʻtloq / Yaylov","lulc_cropland":"Ekin maydoni","lulc_urban":"Qurilgan / Shahar","lulc_sparse":"Ochiq / Siyrak oʻsimlik","lulc_snow":"Qor / Muz","lulc_water":"Suv havzasi","lulc_wetland":"Botqoq / Marsh","lulc_mangrove":"Mangrov","lulc_moss":"Moxlar / Likenlar","modis_evergreen_needle":"Doim yashil ignabargli oʻrmon","modis_evergreen_broad":"Doim yashil keng bargli oʻrmon","modis_deciduous_needle":"Bargini toʻkuvchi ignabargli oʻrmon","modis_deciduous_broad":"Bargini toʻkuvchi keng bargli oʻrmon","modis_mixed_forest":"Aralash oʻrmonlar","modis_closed_shrub":"Yopiq butazorlar","modis_open_shrub":"Ochiq butazorlar","modis_woody_savanna":"Daraxtzor savannalar","modis_savanna":"Savannalar","modis_grassland":"Oʻtloqlar","modis_wetland":"Doimiy botqoqliklar","modis_cropland":"Ekin maydonlari","modis_urban":"Shahar va qurilgan hudud","modis_crop_natural":"Ekin maydoni / Tabiiy oʻsimlik mozaikasi","modis_snow":"Qor va muz","modis_bare":"Yalang yoki siyrak oʻsimlikli","modis_water":"Suv havzalari","clc_urban_cont":"Uzluksiz shahar toʻqimasi","clc_urban_disc":"Uzuq shahar toʻqimasi","clc_industrial":"Sanoat / Tijorat","clc_road_rail":"Yoʻl / Temir yoʻl tarmoqlari","clc_port":"Port hududlari","clc_airport":"Aeroportlar","clc_mine":"Kon qazib olish joylari","clc_dump":"Chiqindixonalar","clc_construction":"Qurilish maydonlari","clc_urban_green":"Shahar yashil hududlari","clc_sport":"Sport / Dam olish","clc_arable_dry":"Sugʻorilmaydigan haydaladigan yer","clc_arable_irr":"Sugʻoriladigan haydaladigan yer","clc_rice":"Sholi maydonlari","clc_vineyard":"Uzumzorlar","clc_fruit":"Mevali daraxtlar","clc_olive":"Zaytunzorlar","clc_pasture":"Yaylovlar","clc_annual_mosaic":"Bir yillik / koʻp yillik ekinlar mozaikasi","clc_complex_farm":"Murakkab dehqonchilik shakllari","clc_agri_natural":"Tabiiy oʻsimlik bilan qishloq xoʻjaligi","clc_agri_forest":"Agro-oʻrmon hududlari","clc_broadleaf":"Keng bargli oʻrmon","clc_conifer":"Ignabargli oʻrmon","clc_mixed_forest":"Aralash oʻrmon","clc_natural_grass":"Tabiiy oʻtloq","clc_moor":"Botqoqli dasht / Vereskzor","clc_sclerophyllous":"Sklerofil oʻsimlik","clc_transitional":"Oraliq oʻrmon-buta","clc_beach":"Plyajlar / Qumtepalar / Qum","clc_bare_rock":"Ochiq qoya","clc_sparse_veg":"Siyrak oʻsimlikli hududlar","clc_burnt":"Kuygan hududlar","clc_glacier":"Muzliklar / Doimiy qor","clc_inland_marsh":"Ichki botqoqliklar","clc_peatbog":"Torfli botqoqliklar","clc_salt_marsh":"Shoʻr botqoqliklar","clc_salines":"Shoʻrxoklar","clc_intertidal":"Suv osti-usti tekisliklari","clc_river":"Suv oqimlari","clc_lake":"Suv havzalari","clc_lagoon":"Qirgʻoq lagunalari","clc_estuary":"Daryo estuariylari","clc_sea":"Dengiz / Okean"},"el":{"lulc_tree":"Δενδροκάλυψη / Δάσος","lulc_shrubland":"Θαμνώδης Έκταση","lulc_grassland":"Χορτολίβαδο / Βοσκότοπος","lulc_cropland":"Καλλιεργήσιμη Γη","lulc_urban":"Δομημένη / Αστική Περιοχή","lulc_sparse":"Γυμνή / Αραιή Βλάστηση","lulc_snow":"Χιόνι / Πάγος","lulc_water":"Υδάτινη Επιφάνεια","lulc_wetland":"Υγρότοπος / Έλος","lulc_mangrove":"Μαγκρόβια Βλάστηση","lulc_moss":"Βρύα / Λειχήνες","modis_evergreen_needle":"Αειθαλές Βελονοφόρο Δάσος","modis_evergreen_broad":"Αειθαλές Πλατύφυλλο Δάσος","modis_deciduous_needle":"Φυλλοβόλο Βελονοφόρο Δάσος","modis_deciduous_broad":"Φυλλοβόλο Πλατύφυλλο Δάσος","modis_mixed_forest":"Μεικτά Δάση","modis_closed_shrub":"Πυκνοί Θαμνώνες","modis_open_shrub":"Αραιοί Θαμνώνες","modis_woody_savanna":"Ξυλώδεις Σαβάνες","modis_savanna":"Σαβάνες","modis_grassland":"Χορτολίβαδα","modis_wetland":"Μόνιμοι Υγρότοποι","modis_cropland":"Καλλιεργήσιμες Εκτάσεις","modis_urban":"Αστικές & Δομημένες Περιοχές","modis_crop_natural":"Μωσαϊκό Καλλιεργειών / Φυσικής Βλάστησης","modis_snow":"Χιόνι και Πάγος","modis_bare":"Γυμνό ή Αραιά Καλυμμένο με Βλάστηση","modis_water":"Υδάτινες Επιφάνειες","clc_urban_cont":"Συνεχής Αστικός Ιστός","clc_urban_disc":"Ασυνεχής Αστικός Ιστός","clc_industrial":"Βιομηχανικές / Εμπορικές Ζώνες","clc_road_rail":"Οδικά / Σιδηροδρομικά Δίκτυα","clc_port":"Λιμενικές Ζώνες","clc_airport":"Αεροδρόμια","clc_mine":"Χώροι Εξόρυξης Ορυκτών","clc_dump":"Χώροι Απόρριψης","clc_construction":"Εργοτάξια","clc_urban_green":"Αστικές Πράσινες Ζώνες","clc_sport":"Αθλητισμός / Αναψυχή","clc_arable_dry":"Μη Αρδευόμενη Αρόσιμη Γη","clc_arable_irr":"Αρδευόμενη Αρόσιμη Γη","clc_rice":"Ορυζώνες","clc_vineyard":"Αμπελώνες","clc_fruit":"Οπωρώνες","clc_olive":"Ελαιώνες","clc_pasture":"Βοσκότοποι","clc_annual_mosaic":"Μωσαϊκό Ετήσιων / Μόνιμων Καλλιεργειών","clc_complex_farm":"Σύνθετα Πρότυπα Καλλιέργειας","clc_agri_natural":"Γεωργία με Φυσική Βλάστηση","clc_agri_forest":"Δασογεωργικές Εκτάσεις","clc_broadleaf":"Πλατύφυλλο Δάσος","clc_conifer":"Κωνοφόρο Δάσος","clc_mixed_forest":"Μεικτό Δάσος","clc_natural_grass":"Φυσικό Χορτολίβαδο","clc_moor":"Χερσότοποι / Ρεικότοποι","clc_sclerophyllous":"Σκληρόφυλλη Βλάστηση","clc_transitional":"Μεταβατική Δασωθαμνώδης Έκταση","clc_beach":"Παραλίες / Αμμόλοφοι / Άμμος","clc_bare_rock":"Γυμνός Βράχος","clc_sparse_veg":"Αραιά Βλαστημένες Περιοχές","clc_burnt":"Καμένες Εκτάσεις","clc_glacier":"Παγετώνες / Αιώνιο Χιόνι","clc_inland_marsh":"Εσωτερικοί Βάλτοι","clc_peatbog":"Τυρφώνες","clc_salt_marsh":"Αλμυροί Βάλτοι","clc_salines":"Αλυκές","clc_intertidal":"Παλιρροϊκές Επίπεδες Εκτάσεις","clc_river":"Υδάτινα Ρεύματα","clc_lake":"Υδάτινες Επιφάνειες","clc_lagoon":"Παράκτιες Λιμνοθάλασσες","clc_estuary":"Εκβολές Ποταμών","clc_sea":"Θάλασσα / Ωκεανός"},"bg":{"lulc_tree":"Дървесна покривка / гора","lulc_shrubland":"Храсталаци","lulc_grassland":"Ливада / пасище","lulc_cropland":"Обработваема земя","lulc_urban":"Урбанизирана територия","lulc_sparse":"Оголена / рядка растителност","lulc_snow":"Сняг / лед","lulc_water":"Водоем","lulc_wetland":"Влажни зони / блато","lulc_mangrove":"Мангрови гори","lulc_moss":"Мъхове / лишеи","modis_evergreen_needle":"Вечнозелена иглолистна гора","modis_evergreen_broad":"Вечнозелена широколистна гора","modis_deciduous_needle":"Листопадна иглолистна гора","modis_deciduous_broad":"Листопадна широколистна гора","modis_mixed_forest":"Смесени гори","modis_closed_shrub":"Гъсти храсталаци","modis_open_shrub":"Редки храсталаци","modis_woody_savanna":"Дървесна савана","modis_savanna":"Савана","modis_grassland":"Ливади","modis_wetland":"Постоянни влажни зони","modis_cropland":"Обработваема земя","modis_urban":"Градска застройка","modis_crop_natural":"Мозайка от ниви и естествена растителност","modis_snow":"Сняг и лед","modis_bare":"Оголена или слабо покрита с растителност земя","modis_water":"Водни обекти","clc_urban_cont":"Непрекъсната градска застройка","clc_urban_disc":"Прекъсната градска застройка","clc_industrial":"Промишлена / търговска зона","clc_road_rail":"Пътна / железопътна мрежа","clc_port":"Пристанищни зони","clc_airport":"Летища","clc_mine":"Територии за добив на полезни изкопаеми","clc_dump":"Сметища","clc_construction":"Строителни площадки","clc_urban_green":"Градски зелени площи","clc_sport":"Спорт / отдих","clc_arable_dry":"Неполивни обработваеми земи","clc_arable_irr":"Поливни обработваеми земи","clc_rice":"Оризища","clc_vineyard":"Лозя","clc_fruit":"Овощни градини","clc_olive":"Маслинови горички","clc_pasture":"Пасища","clc_annual_mosaic":"Мозайка от едногодишни / многогодишни култури","clc_complex_farm":"Сложни земеделски системи","clc_agri_natural":"Земеделски земи с елементи на естествена растителност","clc_agri_forest":"Агро-горски територии","clc_broadleaf":"Широколистна гора","clc_conifer":"Иглолистна гора","clc_mixed_forest":"Смесена гора","clc_natural_grass":"Естествени ливади","clc_moor":"Пирищни / торфени земи","clc_sclerophyllous":"Склерофилна растителност","clc_transitional":"Преходни горско-храстови територии","clc_beach":"Плажове / дюни / пясъци","clc_bare_rock":"Оголени скали","clc_sparse_veg":"Територии с рядка растителност","clc_burnt":"Изгорели територии","clc_glacier":"Ледници / вечни снегове","clc_inland_marsh":"Вътрешни блата","clc_peatbog":"Торфени блата","clc_salt_marsh":"Солени блата","clc_salines":"Солници","clc_intertidal":"Приливни плитчини","clc_river":"Водни течения","clc_lake":"Водоеми","clc_lagoon":"Крайбрежни лагуни","clc_estuary":"Естуари","clc_sea":"Море / океан"},"tr":{"lulc_tree":"Ağaç Örtüsü / Orman","lulc_shrubland":"Çalılık","lulc_grassland":"Çayır / Otlak","lulc_cropland":"Tarım Alanı","lulc_urban":"Yapay / Kentsel Alan","lulc_sparse":"Çıplak / Seyrek Bitki Örtüsü","lulc_snow":"Kar / Buz","lulc_water":"Su Kütlesi","lulc_wetland":"Sulak Alan / Bataklık","lulc_mangrove":"Mangrov","lulc_moss":"Yosun / Liken","modis_evergreen_needle":"Herdemyeşil İbreli Orman","modis_evergreen_broad":"Herdemyeşil Geniş Yapraklı Orman","modis_deciduous_needle":"Yaprak Döken İbreli Orman","modis_deciduous_broad":"Yaprak Döken Geniş Yapraklı Orman","modis_mixed_forest":"Karışık Ormanlar","modis_closed_shrub":"Kapalı Çalılık","modis_open_shrub":"Açık Çalılık","modis_woody_savanna":"Odunlu Savana","modis_savanna":"Savana","modis_grassland":"Çayır / Otlak","modis_wetland":"Kalıcı Sulak Alan","modis_cropland":"Tarım Alanı","modis_urban":"Kentsel / Yapay Alan","modis_crop_natural":"Tarım-Doğal Mozaik","modis_snow":"Kar ve Buz","modis_bare":"Çıplak Toprak / Seyrek Örtü","modis_water":"Su Kütlesi","clc_urban_cont":"Sürekli Kentsel Doku","clc_urban_disc":"Süreksiz Kentsel Doku","clc_industrial":"Sanayi / Ticaret","clc_road_rail":"Yol / Demiryolu","clc_port":"Liman Alanları","clc_airport":"Havalimanları","clc_mine":"Maden Çıkarım Sahası","clc_dump":"Döküm / Atık Sahası","clc_construction":"İnşaat Sahası","clc_urban_green":"Kentsel Yeşil Alan","clc_sport":"Spor / Eğlence","clc_arable_dry":"Sulanmayan Tarım","clc_arable_irr":"Sulanan Tarım","clc_rice":"Pirinç Tarlaları","clc_vineyard":"Bağlar","clc_fruit":"Meyve Bahçeleri","clc_olive":"Zeytin Bahçeleri","clc_pasture":"Çayır / Mera","clc_annual_mosaic":"Yıllık Tarım Mozaiği","clc_complex_farm":"Karmaşık Tarım","clc_agri_natural":"Tarım-Doğal Mozaik","clc_agri_forest":"Tarım-Ormanlık Mozaik","clc_broadleaf":"Geniş Yapraklı Orman","clc_conifer":"İbreli Orman","clc_mixed_forest":"Karışık Orman","clc_natural_grass":"Doğal Çayırlık","clc_moor":"Bozkır / Fundalık","clc_sclerophyllous":"Makiler","clc_transitional":"Geçiş Orman-Çalılık","clc_beach":"Plaj / Kum / Dün","clc_bare_rock":"Çıplak Kayalık","clc_sparse_veg":"Seyrek Bitki Örtüsü","clc_burnt":"Yanmış Alan","clc_glacier":"Buzul / Kalıcı Kar","clc_inland_marsh":"İç Bataklık","clc_peatbog":"Turbalık","clc_salt_marsh":"Tuz Bataklığı","clc_salines":"Tuzla","clc_intertidal":"Gelgit Düzlüğü","clc_river":"Akarsu","clc_lake":"Göl / Gölet","clc_lagoon":"Kıyı Lagünü","clc_estuary":"Haliç","clc_sea":"Deniz / Okyanus"},"az":{"lulc_tree":"Ağac Örtüyü / Meşə","lulc_shrubland":"Kolluq","lulc_grassland":"Çəmən / Otlaq","lulc_cropland":"Əkin Sahəsi","lulc_urban":"Süni / Şəhər Sahəsi","lulc_sparse":"Çılpaq / Seyrək Bitki Örtüyü","lulc_snow":"Qar / Buz","lulc_water":"Su Hövzəsi","lulc_wetland":"Sulaq Sahə / Bataqlıq","lulc_mangrove":"Manqrov","lulc_moss":"Mamır / Şibyə","modis_evergreen_needle":"Həmişəyaşıl İynəyarpaqlı Meşə","modis_evergreen_broad":"Həmişəyaşıl Enliyarpaqlı Meşə","modis_deciduous_needle":"Yarpaqdökən İynəyarpaqlı Meşə","modis_deciduous_broad":"Yarpaqdökən Enliyarpaqlı Meşə","modis_mixed_forest":"Qarışıq Meşələr","modis_closed_shrub":"Qapalı Kolluq","modis_open_shrub":"Açıq Kolluq","modis_woody_savanna":"Ağaçlı Savanna","modis_savanna":"Savanna","modis_grassland":"Çəmən / Otlaq","modis_wetland":"Daimi Sulaq Sahə","modis_cropland":"Əkin Sahəsi","modis_urban":"Şəhər / Süni Sahə","modis_crop_natural":"Əkin-Təbii Mozaika","modis_snow":"Qar və Buz","modis_bare":"Çılpaq Torpaq / Seyrək Örtük","modis_water":"Su Hövzəsi","clc_urban_cont":"Davamlı Şəhər Toxuması","clc_urban_disc":"Fasiləli Şəhər Toxuması","clc_industrial":"Sənaye / Ticarət","clc_road_rail":"Yol / Dəmir Yolu","clc_port":"Liman Sahələri","clc_airport":"Hava Limanları","clc_mine":"Mədən Çıxarma Sahəsi","clc_dump":"Zibil / Tullantı Sahəsi","clc_construction":"İnşaat Sahəsi","clc_urban_green":"Şəhər Yaşıl Sahəsi","clc_sport":"İdman / Əyləncə","clc_arable_dry":"Suvarılmayan Əkin","clc_arable_irr":"Suvarılan Əkin","clc_rice":"Düyü Tarlaları","clc_vineyard":"Bağlar","clc_fruit":"Meyvə Bağları","clc_olive":"Zeytun Bağları","clc_pasture":"Çəmən / Otlaq","clc_annual_mosaic":"İllik Əkin Mozaikası","clc_complex_farm":"Mürəkkəb Əkinçilik","clc_agri_natural":"Əkin-Təbii Mozaika","clc_agri_forest":"Əkin-Meşəlik Mozaika","clc_broadleaf":"Enliyarpaqlı Meşə","clc_conifer":"İynəyarpaqlı Meşə","clc_mixed_forest":"Qarışıq Meşə","clc_natural_grass":"Təbii Çəmənlik","clc_moor":"Bozqır / Kolluq","clc_sclerophyllous":"Makilər","clc_transitional":"Keçid Meşə-Kolluq","clc_beach":"Çimərlik / Qum / Dünə","clc_bare_rock":"Çılpaq Qayalıq","clc_sparse_veg":"Seyrək Bitki Örtüyü","clc_burnt":"Yanmış Sahə","clc_glacier":"Buzlaq / Daimi Qar","clc_inland_marsh":"Daxili Bataqlıq","clc_peatbog":"Torflaq","clc_salt_marsh":"Duz Bataqlığı","clc_salines":"Duzlaq","clc_intertidal":"Qabarma-çəkilmə Düzənliyi","clc_river":"Çay","clc_lake":"Göl / Hovuz","clc_lagoon":"Sahil Laqunu","clc_estuary":"Çay mənbəyi","clc_sea":"Dəniz / Okean"},"hu":{"lulc_tree":"Fás Terület / Erdő","lulc_shrubland":"Cserjés","lulc_grassland":"Gyep / Legelő","lulc_cropland":"Mezőgazdasági Terület","lulc_urban":"Mesterséges / Városi Terület","lulc_sparse":"Csupasz / Ritka Növényzet","lulc_snow":"Hó / Jég","lulc_water":"Vízfelület","lulc_wetland":"Vizes Élőhely / Mocsár","lulc_mangrove":"Mangrove","lulc_moss":"Moha / Zuzmó","modis_evergreen_needle":"Örökzöld Tűlevelű Erdő","modis_evergreen_broad":"Örökzöld Lombos Erdő","modis_deciduous_needle":"Lombhullató Tűlevelű Erdő","modis_deciduous_broad":"Lombhullató Lombos Erdő","modis_mixed_forest":"Vegyes Erdők","modis_closed_shrub":"Zárt Cserjés","modis_open_shrub":"Nyílt Cserjés","modis_woody_savanna":"Fás Szavanna","modis_savanna":"Szavanna","modis_grassland":"Gyep / Legelő","modis_wetland":"Állandó Vizes Élőhely","modis_cropland":"Mezőgazdasági Terület","modis_urban":"Városi / Mesterséges Terület","modis_crop_natural":"Mezőgazdasági-Természetes Mozaik","modis_snow":"Hó és Jég","modis_bare":"Csupasz Talaj / Ritka Fedettség","modis_water":"Vízfelület","clc_urban_cont":"Összefüggő Városi Szövet","clc_urban_disc":"Nem Összefüggő Városi Szövet","clc_industrial":"Ipar / Kereskedelem","clc_road_rail":"Út / Vasút","clc_port":"Kikötőterületek","clc_airport":"Repülőterek","clc_mine":"Bányaterület","clc_dump":"Hulladéklerakó","clc_construction":"Építési Terület","clc_urban_green":"Városi Zöldterület","clc_sport":"Sport / Szabadidő","clc_arable_dry":"Nem Öntözött Szántó","clc_arable_irr":"Öntözött Szántó","clc_rice":"Rizsföldek","clc_vineyard":"Szőlők","clc_fruit":"Gyümölcsösök","clc_olive":"Olajfaligetek","clc_pasture":"Legelő","clc_annual_mosaic":"Éves Mezőgazdasági Mozaik","clc_complex_farm":"Komplex Mezőgazdaság","clc_agri_natural":"Mezőgazdasági-Természetes Mozaik","clc_agri_forest":"Mezőgazdasági-Erdős Mozaik","clc_broadleaf":"Lombos Erdő","clc_conifer":"Tűlevelű Erdő","clc_mixed_forest":"Vegyes Erdő","clc_natural_grass":"Természetes Gyepterület","clc_moor":"Puszta / Cserjés","clc_sclerophyllous":"Keménylombú Cserjés","clc_transitional":"Átmeneti Erdő-Cserjés","clc_beach":"Strand / Homok / Dűne","clc_bare_rock":"Csupasz Kőzet","clc_sparse_veg":"Ritka Növényzet","clc_burnt":"Leégett Terület","clc_glacier":"Gleccser / Állandó Hó","clc_inland_marsh":"Belvízi Mocsár","clc_peatbog":"Tőzegláp","clc_salt_marsh":"Sós Mocsár","clc_salines":"Sóspárologtató","clc_intertidal":"Ártéri Zátony","clc_river":"Folyó","clc_lake":"Tó / Tavacska","clc_lagoon":"Part Menti Lagúna","clc_estuary":"Torkolat","clc_sea":"Tenger / Óceán"},"kk":{"lulc_tree":"Ағаш жамылғысы / Орман","lulc_shrubland":"Бұталар","lulc_grassland":"Шалғын / Жайылым","lulc_cropland":"Егістік жер","lulc_urban":"Қалалық / Құрылыс аймағы","lulc_sparse":"Жалаңаш / Сирек өсімдік","lulc_snow":"Қар / Мұз","lulc_water":"Су беті","lulc_wetland":"Батпақ / Сулы жер","lulc_mangrove":"Мангр","lulc_moss":"Мох / Қыналар","modis_evergreen_needle":"Мәңгі жасыл қылқан жапырақты орман","modis_evergreen_broad":"Мәңгі жасыл жалпақ жапырақты орман","modis_deciduous_needle":"Жапырағын төгетін қылқан жапырақты орман","modis_deciduous_broad":"Жапырағын төгетін жалпақ жапырақты орман","modis_mixed_forest":"Аралас орман","modis_closed_shrub":"Тығыз бұталар","modis_open_shrub":"Ашық бұталар","modis_woody_savanna":"Ағашты саванна","modis_savanna":"Саванна","modis_grassland":"Шалғын","modis_wetland":"Тұрақты батпақ","modis_cropland":"Егістік жер","modis_urban":"Қалалық / Құрылыс аймағы","modis_crop_natural":"Ауыл шаруашылығы–табиғи мозаика","modis_snow":"Қар мен мұз","modis_bare":"Жалаңаш жер / Сирек жамылғы","modis_water":"Су беті","clc_urban_cont":"Үздіксіз қалалық аймақ","clc_urban_disc":"Үзіктілі қалалық аймақ","clc_industrial":"Өнеркәсіп / Сауда","clc_road_rail":"Жол / Темір жол желісі","clc_port":"Порт аймақтары","clc_airport":"Әуежайлар","clc_mine":"Тау-кен аймақтары","clc_dump":"Қоқыс полигондары","clc_construction":"Құрылыс аймақтары","clc_urban_green":"Қалалық жасыл аймақтар","clc_sport":"Спорт / Демалыс","clc_arable_dry":"Суармасыз егістік","clc_arable_irr":"Суармалы егістік","clc_rice":"Күріш алқаптары","clc_vineyard":"Жүзім бақтары","clc_fruit":"Жеміс бақтары","clc_olive":"Зәйтүн бақтары","clc_pasture":"Жайылымдар","clc_annual_mosaic":"Жылдық / Көпжылдық мозаика","clc_complex_farm":"Күрделі егіншілік жүйесі","clc_agri_natural":"Табиғи өсімдіктермен ауыл шаруашылығы","clc_agri_forest":"Ауыл шаруашылығы–орман аймағы","clc_broadleaf":"Жалпақ жапырақты орман","clc_conifer":"Қылқан жапырақты орман","clc_mixed_forest":"Аралас орман","clc_natural_grass":"Табиғи шалғын","clc_moor":"Дала / Бұталар","clc_sclerophyllous":"Қатты жапырақты өсімдік","clc_transitional":"Орман–бұталар өтпелі аймағы","clc_beach":"Жағажай / Құм / Дюна","clc_bare_rock":"Жалаңаш жар","clc_sparse_veg":"Сирек өсімдік","clc_burnt":"Өртенген аймақ","clc_glacier":"Мұздық / Тұрақты қар","clc_inland_marsh":"Ішкі батпақ","clc_peatbog":"Торфты батпақ","clc_salt_marsh":"Тұзды батпақ","clc_salines":"Тұзды алаптар","clc_intertidal":"Тасқын аралық аймақ","clc_river":"Өзен / Ағын","clc_lake":"Көл","clc_lagoon":"Жағалаулық лагуна","clc_estuary":"Өзен сағасы","clc_sea":"Теңіз / Мұхит"},"de":{"lulc_tree":"Baumbedeckung / Wald","lulc_shrubland":"Buschland","lulc_grassland":"Grünland / Weide","lulc_cropland":"Ackerfläche","lulc_urban":"Bebaut / Urban","lulc_sparse":"Kahl / Spärliche Vegetation","lulc_snow":"Schnee / Eis","lulc_water":"Wasserkörper","lulc_wetland":"Feuchtgebiet / Sumpf","lulc_mangrove":"Mangrove","lulc_moss":"Moos / Flechte","modis_evergreen_needle":"Immergrüner Nadelwald","modis_evergreen_broad":"Immergrüner Laubwald","modis_deciduous_needle":"Laubabwerfender Nadelwald","modis_deciduous_broad":"Laubabwerfender Laubwald","modis_mixed_forest":"Mischwald","modis_closed_shrub":"Geschlossenes Strauchland","modis_open_shrub":"Offenes Strauchland","modis_woody_savanna":"Holzige Savanne","modis_savanna":"Savanne","modis_grassland":"Grasland","modis_wetland":"Dauerhaftes Feuchtgebiet","modis_cropland":"Ackerfläche","modis_urban":"Stadtgebiet / Bebaut","modis_crop_natural":"Acker-Naturvegetation-Mosaik","modis_snow":"Schnee und Eis","modis_bare":"Kahl oder Spärlich Bewachsen","modis_water":"Wasserkörper","clc_urban_cont":"Dicht bebaute Stadtflächen","clc_urban_disc":"Locker bebaute Stadtflächen","clc_industrial":"Industrie / Gewerbe","clc_road_rail":"Straßen / Schienen","clc_port":"Hafengebiete","clc_airport":"Flughäfen","clc_mine":"Abgrabungsflächen","clc_dump":"Deponien / Müllkippen","clc_construction":"Baustellen","clc_urban_green":"Städtische Grünflächen","clc_sport":"Sport- / Freizeitflächen","clc_arable_dry":"Nicht bewässertes Ackerland","clc_arable_irr":"Bewässertes Ackerland","clc_rice":"Reisfelder","clc_vineyard":"Weinbauflächen","clc_fruit":"Obst- und Beerengärten","clc_olive":"Olivenhaine","clc_pasture":"Wiesen / Weiden","clc_annual_mosaic":"Einjährige / Dauerkulturen (Mosaik)","clc_complex_farm":"Komplexe Parzellenstruktur","clc_agri_natural":"Landwirtschaft mit natürlicher Vegetation","clc_agri_forest":"Agroforstwirtschaft","clc_broadleaf":"Laubwälder","clc_conifer":"Nadelwälder","clc_mixed_forest":"Mischwälder","clc_natural_grass":"Natürliches Grünland","clc_moor":"Heiden / Moorheiden","clc_sclerophyllous":"Hartlaubvegetation","clc_transitional":"Wald-Strauch-Übergangsstadien","clc_beach":"Strände / Dünen / Sand","clc_bare_rock":"Fels ohne Vegetation","clc_sparse_veg":"Spärliche Vegetation","clc_burnt":"Brandflächen","clc_glacier":"Gletscher / Ewiger Schnee","clc_inland_marsh":"Binnengewässer-Sümpfe","clc_peatbog":"Torfmoore","clc_salt_marsh":"Salzsümpfe","clc_salines":"Salzwiesen / Salzebenen","clc_intertidal":"Wattflächen","clc_river":"Fließgewässer","clc_lake":"Seen / Teiche","clc_lagoon":"Küstenlagunen","clc_estuary":"Flussmündungen","clc_sea":"Meer / Ozean"},"fr":{"lulc_tree":"Couvert arboré / Forêt","lulc_shrubland":"Arbustes","lulc_grassland":"Prairie / Pâturage","lulc_cropland":"Terres agricoles","lulc_urban":"Construit / Urbain","lulc_sparse":"Nu / Végétation éparse","lulc_snow":"Neige / Glace","lulc_water":"Plan d\\","lulc_wetland":"Zone humide / Marais","lulc_mangrove":"Mangrove","lulc_moss":"Mousse / Lichen","modis_evergreen_needle":"Forêt de conifères persistants","modis_evergreen_broad":"Forêt de feuillus persistants","modis_deciduous_needle":"Forêt de conifères caducs","modis_deciduous_broad":"Forêt de feuillus caducs","modis_mixed_forest":"Forêts mixtes","modis_closed_shrub":"Broussailles fermées","modis_open_shrub":"Broussailles ouvertes","modis_woody_savanna":"Savane arborée","modis_savanna":"Savane","modis_grassland":"Prairie","modis_wetland":"Zones humides permanentes","modis_cropland":"Terres cultivées","modis_urban":"Urbain / Construit","modis_crop_natural":"Mosaïque cultures / végétation naturelle","modis_snow":"Neige et glace","modis_bare":"Nu ou végétation éparse","modis_water":"Plan d'eau","clc_urban_cont":"Tissu urbain continu","clc_urban_disc":"Tissu urbain discontinu","clc_industrial":"Zones industrielles / commerciales","clc_road_rail":"Réseaux routiers / ferroviaires","clc_port":"Zones portuaires","clc_airport":"Aéroports","clc_mine":"Extraction de matériaux","clc_dump":"Décharges","clc_construction":"Chantiers de construction","clc_urban_green":"Espaces verts urbains","clc_sport":"Équipements sportifs / loisirs","clc_arable_dry":"Terres arables non irriguées","clc_arable_irr":"Terres arables irriguées","clc_rice":"Rizières","clc_vineyard":"Vignobles","clc_fruit":"Vergers / Petits fruits","clc_olive":"Oliveraies","clc_pasture":"Prairies","clc_annual_mosaic":"Cultures annuelles / permanentes (mosaïque)","clc_complex_farm":"Systèmes culturaux complexes","clc_agri_natural":"Agriculture avec végétation naturelle","clc_agri_forest":"Zones agro-forestières","clc_broadleaf":"Forêts de feuillus","clc_conifer":"Forêts de conifères","clc_mixed_forest":"Forêts mixtes","clc_natural_grass":"Pelouses naturelles","clc_moor":"Landes et broussailles","clc_sclerophyllous":"Végétation sclérophylle","clc_transitional":"Forêt / végétation arbustive en mutation","clc_beach":"Plages / dunes / sable","clc_bare_rock":"Roches nues","clc_sparse_veg":"Végétation clairsemée","clc_burnt":"Zones incendiées","clc_glacier":"Glaciers / neige éternelle","clc_inland_marsh":"Marais intérieurs","clc_peatbog":"Tourbières","clc_salt_marsh":"Marais salants","clc_salines":"Salines","clc_intertidal":"Vasières intertidales","clc_river":"Cours d'eau","clc_lake":"Plans d'eau","clc_lagoon":"Lagunes côtières","clc_estuary":"Estuaires","clc_sea":"Mer / Océan"},"es":{"lulc_tree":"Cubierta arbórea / Bosque","lulc_shrubland":"Matorral","lulc_grassland":"Pastizal / Pradera","lulc_cropland":"Tierra de cultivo","lulc_urban":"Construido / Urbano","lulc_sparse":"Desnudo / Vegetación escasa","lulc_snow":"Nieve / Hielo","lulc_water":"Cuerpo de agua","lulc_wetland":"Humedal / Pantano","lulc_mangrove":"Manglar","lulc_moss":"Musgo / Liquen","modis_evergreen_needle":"Bosque de coníferas perennifolio","modis_evergreen_broad":"Bosque de hoja ancha perennifolio","modis_deciduous_needle":"Bosque de coníferas caducifolio","modis_deciduous_broad":"Bosque de hoja ancha caducifolio","modis_mixed_forest":"Bosque mixto","modis_closed_shrub":"Matorral cerrado","modis_open_shrub":"Matorral abierto","modis_woody_savanna":"Sabana arbolada","modis_savanna":"Sabana","modis_grassland":"Pastizal","modis_wetland":"Humedal permanente","modis_cropland":"Tierras de cultivo","modis_urban":"Urbano / Construido","modis_crop_natural":"Mosaico cultivo / vegetación natural","modis_snow":"Nieve y hielo","modis_bare":"Desnudo o vegetación escasa","modis_water":"Cuerpo de agua","clc_urban_cont":"Tejido urbano continuo","clc_urban_disc":"Tejido urbano discontinuo","clc_industrial":"Industria / Comercio","clc_road_rail":"Redes viarias / ferroviarias","clc_port":"Zonas portuarias","clc_airport":"Aeropuertos","clc_mine":"Zonas de extracción minera","clc_dump":"Vertederos","clc_construction":"Zonas en construcción","clc_urban_green":"Zonas verdes urbanas","clc_sport":"Instalaciones deportivas / recreativas","clc_arable_dry":"Tierras de secano","clc_arable_irr":"Tierras de regadío","clc_rice":"Arrozales","clc_vineyard":"Viñedos","clc_fruit":"Frutales","clc_olive":"Olivares","clc_pasture":"Praderas","clc_annual_mosaic":"Mosaico de cultivos anuales / permanentes","clc_complex_farm":"Mosaico de cultivos complejos","clc_agri_natural":"Agricultura con vegetación natural","clc_agri_forest":"Sistemas agroforestales","clc_broadleaf":"Bosques de frondosas","clc_conifer":"Bosques de coníferas","clc_mixed_forest":"Bosques mixtos","clc_natural_grass":"Pastizales naturales","clc_moor":"Brezales / Matorrales","clc_sclerophyllous":"Vegetación esclerófila","clc_transitional":"Bosques en transición","clc_beach":"Playas / Dunas / Arenas","clc_bare_rock":"Roquedos","clc_sparse_veg":"Vegetación escasa","clc_burnt":"Espacios quemados","clc_glacier":"Glaciares / Nieves permanentes","clc_inland_marsh":"Pantanos y zonas pantanosas","clc_peatbog":"Turberas","clc_salt_marsh":"Marismas","clc_salines":"Salinas","clc_intertidal":"Zonas intermareales","clc_river":"Cursos de agua","clc_lake":"Láminas de agua","clc_lagoon":"Lagunas costeras","clc_estuary":"Estuarios","clc_sea":"Mar / Océano"},"pt":{"lulc_tree":"Coberto arbóreo / Floresta","lulc_shrubland":"Matagal","lulc_grassland":"Pastagem / Prado","lulc_cropland":"Terra de cultivo","lulc_urban":"Edificado / Urbano","lulc_sparse":"Nu / Vegetação esparsa","lulc_snow":"Neve / Gelo","lulc_water":"Massa de água","lulc_wetland":"Zona húmida / Pântano","lulc_mangrove":"Mangal","lulc_moss":"Musgo / Líquen","modis_evergreen_needle":"Floresta de coníferas perenifólias","modis_evergreen_broad":"Floresta de folha larga perenifólia","modis_deciduous_needle":"Floresta de coníferas caducifólias","modis_deciduous_broad":"Floresta de folha larga caducifólia","modis_mixed_forest":"Floresta mista","modis_closed_shrub":"Matagal fechado","modis_open_shrub":"Matagal aberto","modis_woody_savanna":"Savana arbórea","modis_savanna":"Savana","modis_grassland":"Pastagem","modis_wetland":"Zona húmida permanente","modis_cropland":"Terras de cultivo","modis_urban":"Urbano / Edificado","modis_crop_natural":"Mosaico cultivo / vegetação natural","modis_snow":"Neve e gelo","modis_bare":"Nu ou vegetação esparsa","modis_water":"Massa de água","clc_urban_cont":"Tecido urbano contínuo","clc_urban_disc":"Tecido urbano descontínuo","clc_industrial":"Indústria / Comércio","clc_road_rail":"Redes rodoviárias / ferroviárias","clc_port":"Zonas portuárias","clc_airport":"Aeroportos","clc_mine":"Áreas de extração mineira","clc_dump":"Aterros","clc_construction":"Áreas em construção","clc_urban_green":"Espaços verdes urbanos","clc_sport":"Instalações desportivas / recreativas","clc_arable_dry":"Terras de sequeiro","clc_arable_irr":"Terras de regadio","clc_rice":"Arrozais","clc_vineyard":"Vinhas","clc_fruit":"Pomares","clc_olive":"Olivais","clc_pasture":"Pastagens","clc_annual_mosaic":"Mosaico de culturas anuais / permanentes","clc_complex_farm":"Mosaico de culturas complexas","clc_agri_natural":"Agricultura com vegetação natural","clc_agri_forest":"Sistemas agroflorestais","clc_broadleaf":"Florestas de folhosas","clc_conifer":"Florestas de coníferas","clc_mixed_forest":"Florestas mistas","clc_natural_grass":"Pastagens naturais","clc_moor":"Charnecas / Matos","clc_sclerophyllous":"Vegetação esclerófila","clc_transitional":"Florestas em transição","clc_beach":"Praias / Dunas / Areais","clc_bare_rock":"Rochas nuas","clc_sparse_veg":"Vegetação esparsa","clc_burnt":"Áreas ardidas","clc_glacier":"Glaciares / Neves permanentes","clc_inland_marsh":"Pântanos e zonas pantanosas","clc_peatbog":"Turfeiras","clc_salt_marsh":"Sapais","clc_salines":"Salinas","clc_intertidal":"Zonas entre-marés","clc_river":"Cursos de água","clc_lake":"Planos de água","clc_lagoon":"Lagoas costeiras","clc_estuary":"Estuários","clc_sea":"Mar / Oceano"},"ar":{"lulc_tree":"غطاء شجري / غابة","lulc_shrubland":"أدغال","lulc_grassland":"مروج / مراعٍ","lulc_cropland":"أرض زراعية","lulc_urban":"مناطق حضرية / مبنية","lulc_sparse":"نباتات متفرقة","lulc_snow":"ثلج / جليد","lulc_water":"مسطح مائي","lulc_wetland":"أرض رطبة / مستنقع","lulc_mangrove":"غابات المانغروف","lulc_moss":"طحلب / حزاز","modis_evergreen_needle":"غابة صنوبرية دائمة الخضرة","modis_evergreen_broad":"غابة عريضة الأوراق دائمة الخضرة","modis_deciduous_needle":"غابة صنوبرية متساقطة الأوراق","modis_deciduous_broad":"غابة عريضة الأوراق متساقطة","modis_mixed_forest":"غابات مختلطة","modis_closed_shrub":"أدغال مغلقة","modis_open_shrub":"أدغال مفتوحة","modis_woody_savanna":"سافانا خشبية","modis_savanna":"سافانا","modis_grassland":"مروج","modis_wetland":"أراضٍ رطبة دائمة","modis_cropland":"أراضٍ زراعية","modis_urban":"حضري / مبني","modis_crop_natural":"فسيفساء زراعي – طبيعي","modis_snow":"ثلج وجليد","modis_bare":"أرض جرداء أو شبه جرداء","modis_water":"مسطح مائي","clc_urban_cont":"نسيج حضري متواصل","clc_urban_disc":"نسيج حضري متقطع","clc_industrial":"مناطق صناعية / تجارية","clc_road_rail":"شبكات طرق / سكك حديدية","clc_port":"مناطق موانئ","clc_airport":"المطارات","clc_mine":"مناطق استخراج المواد","clc_dump":"مكبات النفايات","clc_construction":"مواقع البناء","clc_urban_green":"مناطق خضراء حضرية","clc_sport":"مرافق الرياضة / الترفيه","clc_arable_dry":"أراضٍ صالحة للزراعة (غير مروية)","clc_arable_irr":"أراضٍ مروية","clc_rice":"حقول الأرز","clc_vineyard":"الكروم","clc_fruit":"بساتين الفاكهة","clc_olive":"مزارع الزيتون","clc_pasture":"المراعي","clc_annual_mosaic":"فسيفساء محاصيل سنوية / دائمة","clc_complex_farm":"أنظمة زراعية معقدة","clc_agri_natural":"زراعة مع غطاء نباتي طبيعي","clc_agri_forest":"مناطق الزراعة الحرجية","clc_broadleaf":"غابات واسعة الأوراق","clc_conifer":"غابات صنوبرية","clc_mixed_forest":"غابات مختلطة","clc_natural_grass":"مراعٍ طبيعية","clc_moor":"أراضٍ صحراوية / غطاء عشبي","clc_sclerophyllous":"غطاء نباتي صلب الأوراق","clc_transitional":"مرحلة انتقالية غابة-شجيرات","clc_beach":"شواطئ / كثبان / رمال","clc_bare_rock":"صخور عارية","clc_sparse_veg":"غطاء نباتي متفرق","clc_burnt":"مناطق محترقة","clc_glacier":"أنهار جليدية / ثلوج دائمة","clc_inland_marsh":"مستنقعات داخلية","clc_peatbog":"مستنقعات العشب","clc_salt_marsh":"مستنقعات الملح","clc_salines":"سبخات / ملاحات","clc_intertidal":"مناطق مدية","clc_river":"مجاري مائية","clc_lake":"مسطحات مائية","clc_lagoon":"بحيرات ساحلية","clc_estuary":"المصبات","clc_sea":"بحر / محيط"},"fa":{"lulc_tree":"پوشش درختی / جنگل","lulc_shrubland":"بوته‌زار","lulc_grassland":"مرتع / چراگاه","lulc_cropland":"زمین کشاورزی","lulc_urban":"مناطق ساخته‌شده / شهری","lulc_sparse":"پوشش کم","lulc_snow":"برف / یخ","lulc_water":"پیکره آبی","lulc_wetland":"تالاب / باتلاق","lulc_mangrove":"مانگرو","lulc_moss":"خزه / گلسنگ","modis_evergreen_needle":"جنگل سوزنی‌برگ همیشه‌سبز","modis_evergreen_broad":"جنگل پهن‌برگ همیشه‌سبز","modis_deciduous_needle":"جنگل سوزنی‌برگ خزان‌دار","modis_deciduous_broad":"جنگل پهن‌برگ خزان‌دار","modis_mixed_forest":"جنگل مختلط","modis_closed_shrub":"بوته‌زار بسته","modis_open_shrub":"بوته‌زار باز","modis_woody_savanna":"ساوانای چوبی","modis_savanna":"ساوانا","modis_grassland":"علفزار","modis_wetland":"تالاب دائمی","modis_cropland":"زمین کشاورزی","modis_urban":"شهری / ساخته‌شده","modis_crop_natural":"موزاییک کشاورزی – طبیعی","modis_snow":"برف و یخ","modis_bare":"بایر یا پوشش گیاهی کم","modis_water":"پیکره آبی","clc_urban_cont":"بافت شهری پیوسته","clc_urban_disc":"بافت شهری ناپیوسته","clc_industrial":"صنعتی / تجاری","clc_road_rail":"شبکه‌های جاده / ریلی","clc_port":"مناطق بندری","clc_airport":"فرودگاه‌ها","clc_mine":"مناطق استخراج معادن","clc_dump":"محل‌های دفع زباله","clc_construction":"مناطق در حال ساخت‌وساز","clc_urban_green":"فضای سبز شهری","clc_sport":"ورزش / تفریح","clc_arable_dry":"زمین‌های دیم (غیر آبی)","clc_arable_irr":"زمین‌های آبی","clc_rice":"شالیزارها","clc_vineyard":"تاکستان‌ها","clc_fruit":"باغ‌های میوه","clc_olive":"زیتون‌کاری","clc_pasture":"مراتع / چراگاه‌ها","clc_annual_mosaic":"موزاییک محصولات سالانه / دائمی","clc_complex_farm":"الگوهای پیچیده کشت","clc_agri_natural":"کشاورزی با پوشش گیاهی طبیعی","clc_agri_forest":"مناطق جنگل‌کشاورزی","clc_broadleaf":"جنگل پهن‌برگ","clc_conifer":"جنگل سوزنی‌برگ","clc_mixed_forest":"جنگل مختلط","clc_natural_grass":"مراتع طبیعی","clc_moor":"بیشه‌زار / زمین بایر","clc_sclerophyllous":"پوشش سخت‌برگ","clc_transitional":"جنگل-بوته در مرحله انتقالی","clc_beach":"ساحل / تپه‌ماسه / شن","clc_bare_rock":"صخره‌های لخت","clc_sparse_veg":"پوشش گیاهی پراکنده","clc_burnt":"مناطق سوخته","clc_glacier":"یخچال‌های طبیعی / برف دائمی","clc_inland_marsh":"مرداب‌های داخلی","clc_peatbog":"زمین‌های باتلاقی","clc_salt_marsh":"مرداب‌های شور","clc_salines":"نمکزار","clc_intertidal":"پهنه‌های جزرومدی","clc_river":"رودخانه‌ها","clc_lake":"دریاچه‌ها / استخرها","clc_lagoon":"لاگون‌های ساحلی","clc_estuary":"مصب‌های رودخانه","clc_sea":"دریا / اقیانوس"},"zh":{"lulc_tree":"树木覆盖 / 森林","lulc_shrubland":"灌木丛","lulc_grassland":"草地 / 牧场","lulc_cropland":"耕地","lulc_urban":"建成区 / 城市","lulc_sparse":"裸露 / 稀疏植被","lulc_snow":"雪 / 冰","lulc_water":"水体","lulc_wetland":"湿地 / 沼泽","lulc_mangrove":"红树林","lulc_moss":"苔藓 / 地衣","modis_evergreen_needle":"常绿针叶林","modis_evergreen_broad":"常绿阔叶林","modis_deciduous_needle":"落叶针叶林","modis_deciduous_broad":"落叶阔叶林","modis_mixed_forest":"混交林","modis_closed_shrub":"密灌丛","modis_open_shrub":"稀灌丛","modis_woody_savanna":"木本萨瓦纳","modis_savanna":"萨瓦纳","modis_grassland":"草地","modis_wetland":"永久湿地","modis_cropland":"农田","modis_urban":"城市 / 建成区","modis_crop_natural":"农田-自然植被镶嵌体","modis_snow":"雪和冰","modis_bare":"裸地或稀疏植被","modis_water":"水体","clc_urban_cont":"连续城市织物","clc_urban_disc":"非连续城市织物","clc_industrial":"工业 / 商业区","clc_road_rail":"道路 / 铁路网络","clc_port":"港口区域","clc_airport":"机场","clc_mine":"采矿区","clc_dump":"垃圾填埋场","clc_construction":"建设用地","clc_urban_green":"城市绿地","clc_sport":"体育 / 休闲设施","clc_arable_dry":"旱地耕地","clc_arable_irr":"灌溉耕地","clc_rice":"水稻田","clc_vineyard":"葡萄园","clc_fruit":"果园","clc_olive":"橄榄园","clc_pasture":"牧场","clc_annual_mosaic":"年生作物 / 永久作物镶嵌体","clc_complex_farm":"复杂耕作系统","clc_agri_natural":"农业与自然植被混合区","clc_agri_forest":"农林复合区","clc_broadleaf":"阔叶林","clc_conifer":"针叶林","clc_mixed_forest":"混交林","clc_natural_grass":"天然草地","clc_moor":"荒原 / 灌木地","clc_sclerophyllous":"硬叶植被","clc_transitional":"过渡性林地-灌木","clc_beach":"沙滩 / 沙丘 / 沙地","clc_bare_rock":"裸岩","clc_sparse_veg":"稀疏植被","clc_burnt":"火烧迹地","clc_glacier":"冰川 / 永久积雪","clc_inland_marsh":"内陆沼泽","clc_peatbog":"泥炭沼泽","clc_salt_marsh":"盐碱沼泽","clc_salines":"盐田","clc_intertidal":"潮间带滩涂","clc_river":"水道","clc_lake":"水体","clc_lagoon":"沿海泻湖","clc_estuary":"河口","clc_sea":"海洋 / 大洋"},"ja":{"lulc_tree":"樹木被覆 / 森林","lulc_shrubland":"低木地","lulc_grassland":"草地 / 牧草地","lulc_cropland":"農地","lulc_urban":"市街地 / 都市","lulc_sparse":"裸地 / 疎植生","lulc_snow":"雪 / 氷","lulc_water":"水域","lulc_wetland":"湿地 / 沼沢地","lulc_mangrove":"マングローブ","lulc_moss":"苔類 / 地衣類","modis_evergreen_needle":"常緑針葉樹林","modis_evergreen_broad":"常緑広葉樹林","modis_deciduous_needle":"落葉針葉樹林","modis_deciduous_broad":"落葉広葉樹林","modis_mixed_forest":"混交林","modis_closed_shrub":"閉鎖低木地","modis_open_shrub":"開放低木地","modis_woody_savanna":"木本サバンナ","modis_savanna":"サバンナ","modis_grassland":"草地","modis_wetland":"永久湿地","modis_cropland":"農地","modis_urban":"都市 / 建成地","modis_crop_natural":"農地・自然植生混在","modis_snow":"雪と氷","modis_bare":"裸地または疎植生","modis_water":"水域","clc_urban_cont":"連続的な市街地","clc_urban_disc":"非連続的な市街地","clc_industrial":"工業・商業地域","clc_road_rail":"道路・鉄道ネットワーク","clc_port":"港湾地域","clc_airport":"空港","clc_mine":"採掘地","clc_dump":"廃棄物処理場","clc_construction":"建設中の土地","clc_urban_green":"都市緑地","clc_sport":"スポーツ・レクリエーション","clc_arable_dry":"非灌漑耕地","clc_arable_irr":"灌漑耕地","clc_rice":"水田","clc_vineyard":"ブドウ畑","clc_fruit":"果樹園","clc_olive":"オリーブ農園","clc_pasture":"牧草地","clc_annual_mosaic":"年間・永年作物モザイク","clc_complex_farm":"複雑な作付体系","clc_agri_natural":"農地と自然植生の混在","clc_agri_forest":"農林混合地域","clc_broadleaf":"広葉樹林","clc_conifer":"針葉樹林","clc_mixed_forest":"混交林","clc_natural_grass":"自然草地","clc_moor":"荒野・ヒース","clc_sclerophyllous":"硬葉植生","clc_transitional":"移行期の林地・低木地","clc_beach":"浜辺・砂丘・砂地","clc_bare_rock":"裸岩","clc_sparse_veg":"まばらな植生","clc_burnt":"焼失地","clc_glacier":"氷河・万年雪","clc_inland_marsh":"内陸湿地","clc_peatbog":"泥炭地","clc_salt_marsh":"塩性湿地","clc_salines":"塩田","clc_intertidal":"干潟","clc_river":"水路","clc_lake":"水域","clc_lagoon":"沿岸潟湖","clc_estuary":"河口","clc_sea":"海・大洋"},"ko":{"lulc_tree":"수목 피복 / 숲","lulc_shrubland":"관목지","lulc_grassland":"초지 / 목초지","lulc_cropland":"경작지","lulc_urban":"건축지 / 도시","lulc_sparse":"나지 / 희박 식생","lulc_snow":"눈 / 얼음","lulc_water":"수역","lulc_wetland":"습지 / 늪","lulc_mangrove":"맹그로브","lulc_moss":"이끼 / 지의류","modis_evergreen_needle":"상록 침엽수림","modis_evergreen_broad":"상록 활엽수림","modis_deciduous_needle":"낙엽 침엽수림","modis_deciduous_broad":"낙엽 활엽수림","modis_mixed_forest":"혼합림","modis_closed_shrub":"폐쇄 관목지","modis_open_shrub":"개방 관목지","modis_woody_savanna":"목본 사바나","modis_savanna":"사바나","modis_grassland":"초지","modis_wetland":"영구 습지","modis_cropland":"농경지","modis_urban":"도시 / 건설지","modis_crop_natural":"농경지-자연식생 모자이크","modis_snow":"눈과 얼음","modis_bare":"나지 또는 희박 식생","modis_water":"수역","clc_urban_cont":"연속 도시 조직","clc_urban_disc":"불연속 도시 조직","clc_industrial":"산업 / 상업 지역","clc_road_rail":"도로 / 철도 네트워크","clc_port":"항구 지역","clc_airport":"공항","clc_mine":"광물 채취 지역","clc_dump":"매립지","clc_construction":"건설 현장","clc_urban_green":"도시 녹지","clc_sport":"스포츠 / 여가 시설","clc_arable_dry":"비관개 경작지","clc_arable_irr":"관개 경작지","clc_rice":"논","clc_vineyard":"포도원","clc_fruit":"과수원","clc_olive":"올리브 농장","clc_pasture":"목초지","clc_annual_mosaic":"연간 / 영구 작물 모자이크","clc_complex_farm":"복잡한 경작 패턴","clc_agri_natural":"농업과 자연 식생 혼합","clc_agri_forest":"농림 혼합 지역","clc_broadleaf":"활엽수림","clc_conifer":"침엽수림","clc_mixed_forest":"혼합림","clc_natural_grass":"자연 초원","clc_moor":"황야 / 관목지","clc_sclerophyllous":"경엽 식생","clc_transitional":"이행 임지-관목","clc_beach":"해변 / 모래언덕 / 모래","clc_bare_rock":"나지","clc_sparse_veg":"희박 식생","clc_burnt":"화소지","clc_glacier":"빙하 / 만년설","clc_inland_marsh":"내륙 습지","clc_peatbog":"이탄 습지","clc_salt_marsh":"염분 습지","clc_salines":"염전","clc_intertidal":"조간대","clc_river":"수로","clc_lake":"수역","clc_lagoon":"연안 석호","clc_estuary":"하구","clc_sea":"바다 / 대양"},"hi":{"lulc_tree":"वृक्ष आवरण / वन","lulc_shrubland":"झाड़ीभूमि","lulc_grassland":"घास मैदान / चारागाह","lulc_cropland":"कृषि भूमि","lulc_urban":"निर्मित / शहरी","lulc_sparse":"नग्न / विरल वनस्पति","lulc_snow":"बर्फ / हिम","lulc_water":"जल निकाय","lulc_wetland":"आर्द्रभूमि / दलदल","lulc_mangrove":"मैंग्रोव","lulc_moss":"काई / लाइकेन","modis_evergreen_needle":"सदाबहार शंकुधारी वन","modis_evergreen_broad":"सदाबहार चौड़े पत्ते का वन","modis_deciduous_needle":"पर्णपाती शंकुधारी वन","modis_deciduous_broad":"पर्णपाती चौड़े पत्ते का वन","modis_mixed_forest":"मिश्रित वन","modis_closed_shrub":"बंद झाड़ी भूमि","modis_open_shrub":"खुली झाड़ी भूमि","modis_woody_savanna":"काष्ठीय सवाना","modis_savanna":"सवाना","modis_grassland":"घास मैदान","modis_wetland":"स्थायी आर्द्रभूमि","modis_cropland":"कृषि भूमि","modis_urban":"शहरी / निर्मित क्षेत्र","modis_crop_natural":"कृषि-प्राकृतिक मोज़ेक","modis_snow":"हिम और बर्फ","modis_bare":"बंजर या विरल वनस्पति","modis_water":"जल निकाय","clc_urban_cont":"सतत शहरी ढांचा","clc_urban_disc":"असतत शहरी ढांचा","clc_industrial":"औद्योगिक / वाणिज्यिक","clc_road_rail":"सड़क / रेल नेटवर्क","clc_port":"बंदरगाह क्षेत्र","clc_airport":"हवाई अड्डे","clc_mine":"खनन क्षेत्र","clc_dump":"डंप साइट","clc_construction":"निर्माण स्थल","clc_urban_green":"शहरी हरित क्षेत्र","clc_sport":"खेल / मनोरंजन","clc_arable_dry":"असिंचित कृषि भूमि","clc_arable_irr":"सिंचित कृषि भूमि","clc_rice":"धान के खेत","clc_vineyard":"अंगूर के बाग","clc_fruit":"फल बाग","clc_olive":"जैतून के बाग","clc_pasture":"चारागाह","clc_annual_mosaic":"वार्षिक / स्थायी फसल मोज़ेक","clc_complex_farm":"जटिल खेती पैटर्न","clc_agri_natural":"कृषि एवं प्राकृतिक वनस्पति मिश्रण","clc_agri_forest":"कृषि-वन क्षेत्र","clc_broadleaf":"चौड़ी पत्ती वन","clc_conifer":"शंकुधारी वन","clc_mixed_forest":"मिश्रित वन","clc_natural_grass":"प्राकृतिक घास मैदान","clc_moor":"हीथलैंड / झाड़ीभूमि","clc_sclerophyllous":"कठोर पत्ती वनस्पति","clc_transitional":"संक्रमणकालीन वन-झाड़ी","clc_beach":"समुद्रतट / टीले / रेत","clc_bare_rock":"नग्न चट्टान","clc_sparse_veg":"विरल वनस्पति","clc_burnt":"जले हुए क्षेत्र","clc_glacier":"हिमनद / स्थायी हिम","clc_inland_marsh":"आंतरिक दलदल","clc_peatbog":"पीट बोग","clc_salt_marsh":"लवण दलदल","clc_salines":"नमकीन भूमि","clc_intertidal":"अंतर्ज्वारीय क्षेत्र","clc_river":"जलधारा","clc_lake":"जल निकाय","clc_lagoon":"तटीय लैगून","clc_estuary":"नदीमुख","clc_sea":"समुद्र / महासागर"},"id":{"lulc_tree":"Tutupan Pohon / Hutan","lulc_shrubland":"Semak","lulc_grassland":"Padang Rumput","lulc_cropland":"Lahan Pertanian","lulc_urban":"Terbangun / Perkotaan","lulc_sparse":"Tanah Kosong / Vegetasi Jarang","lulc_snow":"Salju / Es","lulc_water":"Badan Air","lulc_wetland":"Lahan Basah / Rawa","lulc_mangrove":"Mangrove","lulc_moss":"Lumut / Lichen","modis_evergreen_needle":"Hutan Jarum Hijau Abadi","modis_evergreen_broad":"Hutan Berdaun Lebar Hijau Abadi","modis_deciduous_needle":"Hutan Jarum Gugur","modis_deciduous_broad":"Hutan Berdaun Lebar Gugur","modis_mixed_forest":"Hutan Campuran","modis_closed_shrub":"Semak Tertutup","modis_open_shrub":"Semak Terbuka","modis_woody_savanna":"Savana Berkayu","modis_savanna":"Savana","modis_grassland":"Padang Rumput","modis_wetland":"Lahan Basah Permanen","modis_cropland":"Lahan Pertanian","modis_urban":"Perkotaan / Terbangun","modis_crop_natural":"Mosaik Pertanian-Alami","modis_snow":"Salju dan Es","modis_bare":"Tandus atau Vegetasi Jarang","modis_water":"Badan Air","clc_urban_cont":"Kawasan Perkotaan Kontinu","clc_urban_disc":"Kawasan Perkotaan Diskontinu","clc_industrial":"Industri / Komersial","clc_road_rail":"Jaringan Jalan / Rel Kereta","clc_port":"Area Pelabuhan","clc_airport":"Bandara","clc_mine":"Area Penambangan","clc_dump":"Tempat Pembuangan Sampah","clc_construction":"Area Konstruksi","clc_urban_green":"Ruang Hijau Perkotaan","clc_sport":"Fasilitas Olahraga / Rekreasi","clc_arable_dry":"Lahan Pertanian Tadah Hujan","clc_arable_irr":"Lahan Pertanian Irigasi","clc_rice":"Sawah","clc_vineyard":"Kebun Anggur","clc_fruit":"Kebun Buah","clc_olive":"Kebun Zaitun","clc_pasture":"Padang Rumput","clc_annual_mosaic":"Mosaik Tanaman Tahunan / Permanen","clc_complex_farm":"Pola Pertanian Kompleks","clc_agri_natural":"Pertanian dengan Vegetasi Alami","clc_agri_forest":"Area Agro-Kehutanan","clc_broadleaf":"Hutan Berdaun Lebar","clc_conifer":"Hutan Konifer","clc_mixed_forest":"Hutan Campuran","clc_natural_grass":"Padang Rumput Alami","clc_moor":"Semak Belukar / Heathland","clc_sclerophyllous":"Vegetasi Sklerofil","clc_transitional":"Lahan Hutan-Semak Transisi","clc_beach":"Pantai / Bukit Pasir / Pasir","clc_bare_rock":"Batuan Gundul","clc_sparse_veg":"Vegetasi Jarang","clc_burnt":"Area Terbakar","clc_glacier":"Gletser / Salju Abadi","clc_inland_marsh":"Rawa Pedalaman","clc_peatbog":"Rawa Gambut","clc_salt_marsh":"Rawa Garam","clc_salines":"Tambak Garam","clc_intertidal":"Dataran Pasang Surut","clc_river":"Sungai / Aliran Air","clc_lake":"Badan Air","clc_lagoon":"Laguna Pantai","clc_estuary":"Muara","clc_sea":"Laut / Samudra"},"ru":{"lulc_tree":"Древесный покров / лес","lulc_shrubland":"Кустарники","lulc_grassland":"Луг / пастбище","lulc_cropland":"Пахотные земли","lulc_urban":"Застроенная территория","lulc_sparse":"Голая / разреженная растительность","lulc_snow":"Снег / лёд","lulc_water":"Водоём","lulc_wetland":"Водно-болотные угодья / болото","lulc_mangrove":"Мангровые заросли","lulc_moss":"Мхи / лишайники","modis_evergreen_needle":"Вечнозелёный хвойный лес","modis_evergreen_broad":"Вечнозелёный широколиственный лес","modis_deciduous_needle":"Листопадный хвойный лес","modis_deciduous_broad":"Листопадный широколиственный лес","modis_mixed_forest":"Смешанные леса","modis_closed_shrub":"Сомкнутые кустарники","modis_open_shrub":"Разреженные кустарники","modis_woody_savanna":"Древесная саванна","modis_savanna":"Саванна","modis_grassland":"Луга","modis_wetland":"Постоянные водно-болотные угодья","modis_cropland":"Пахотные земли","modis_urban":"Городская застройка","modis_crop_natural":"Мозаика пашни и естественной растительности","modis_snow":"Снег и лёд","modis_bare":"Голая или слабо покрытая растительностью земля","modis_water":"Водные объекты","clc_urban_cont":"Сплошная городская застройка","clc_urban_disc":"Разрозненная городская застройка","clc_industrial":"Промышленная / коммерческая зона","clc_road_rail":"Автомобильные / железные дороги","clc_port":"Портовые зоны","clc_airport":"Аэропорты","clc_mine":"Участки добычи полезных ископаемых","clc_dump":"Свалки","clc_construction":"Строительные площадки","clc_urban_green":"Городские зелёные зоны","clc_sport":"Спорт / отдых","clc_arable_dry":"Неорошаемые пахотные земли","clc_arable_irr":"Орошаемые пахотные земли","clc_rice":"Рисовые поля","clc_vineyard":"Виноградники","clc_fruit":"Фруктовые сады","clc_olive":"Оливковые рощи","clc_pasture":"Пастбища","clc_annual_mosaic":"Мозаика однолетних / многолетних культур","clc_complex_farm":"Сложные системы земледелия","clc_agri_natural":"Сельхозугодья с элементами природной растительности","clc_agri_forest":"Агролесные территории","clc_broadleaf":"Широколиственный лес","clc_conifer":"Хвойный лес","clc_mixed_forest":"Смешанный лес","clc_natural_grass":"Естественные луга","clc_moor":"Верещатники / торфяники","clc_sclerophyllous":"Склерофильная растительность","clc_transitional":"Переходные лесо-кустарниковые территории","clc_beach":"Пляжи / дюны / пески","clc_bare_rock":"Голые скалы","clc_sparse_veg":"Территории с разреженной растительностью","clc_burnt":"Выгоревшие территории","clc_glacier":"Ледники / вечные снега","clc_inland_marsh":"Внутренние болота","clc_peatbog":"Торфяные болота","clc_salt_marsh":"Солончаковые болота","clc_salines":"Солончаки","clc_intertidal":"Приливные отмели","clc_river":"Водотоки","clc_lake":"Водоёмы","clc_lagoon":"Прибрежные лагуны","clc_estuary":"Эстуарии","clc_sea":"Море / океан"},"th":{"lulc_tree":"ต้นไม้ / ป่าไม้","lulc_shrubland":"พุ่มไม้","lulc_grassland":"ทุ่งหญ้า / ทุ่งเลี้ยงสัตว์","lulc_cropland":"พื้นที่เกษตร","lulc_urban":"พื้นที่ก่อสร้าง / เมือง","lulc_sparse":"ดินเปล่า / พืชพรรณเบาบาง","lulc_snow":"หิมะ / น้ำแข็ง","lulc_water":"แหล่งน้ำ","lulc_wetland":"พื้นที่ชุ่มน้ำ / หนอง","lulc_mangrove":"ป่าชายเลน","lulc_moss":"มอส / ไลเคน","modis_evergreen_needle":"ป่าสนใบแหลมไม่ผลัดใบ","modis_evergreen_broad":"ป่าใบกว้างไม่ผลัดใบ","modis_deciduous_needle":"ป่าสนใบแหลมผลัดใบ","modis_deciduous_broad":"ป่าใบกว้างผลัดใบ","modis_mixed_forest":"ป่าผสม","modis_closed_shrub":"พุ่มไม้หนาแน่น","modis_open_shrub":"พุ่มไม้โปร่ง","modis_woody_savanna":"ทุ่งหญ้าสะวันนาไม้","modis_savanna":"ทุ่งหญ้าสะวันนา","modis_grassland":"ทุ่งหญ้า","modis_wetland":"พื้นที่ชุ่มน้ำถาวร","modis_cropland":"พื้นที่เพาะปลูก","modis_urban":"เมืองและพื้นที่ก่อสร้าง","modis_crop_natural":"โมเสกเกษตร / พืชพรรณธรรมชาติ","modis_snow":"หิมะและน้ำแข็ง","modis_bare":"ดินโล่งหรือพืชพรรณเบาบาง","modis_water":"แหล่งน้ำ","clc_urban_cont":"ผืนเมืองต่อเนื่อง","clc_urban_disc":"ผืนเมืองไม่ต่อเนื่อง","clc_industrial":"อุตสาหกรรม / พาณิชยกรรม","clc_road_rail":"ถนน / ทางรถไฟ","clc_port":"พื้นที่ท่าเรือ","clc_airport":"สนามบิน","clc_mine":"พื้นที่ขุดแร่","clc_dump":"บ่อขยะ","clc_construction":"พื้นที่ก่อสร้าง","clc_urban_green":"พื้นที่สีเขียวในเมือง","clc_sport":"กีฬา / นันทนาการ","clc_arable_dry":"พื้นที่เพาะปลูกที่ไม่ชลประทาน","clc_arable_irr":"พื้นที่เพาะปลูกที่ชลประทาน","clc_rice":"นาข้าว","clc_vineyard":"ไร่องุ่น","clc_fruit":"สวนผลไม้","clc_olive":"สวนมะกอก","clc_pasture":"ทุ่งหญ้าเลี้ยงสัตว์","clc_annual_mosaic":"โมเสกพืชผลประจำปี / ยืนต้น","clc_complex_farm":"รูปแบบการเพาะปลูกที่ซับซ้อน","clc_agri_natural":"เกษตรกับพืชพรรณธรรมชาติ","clc_agri_forest":"พื้นที่วนเกษตร","clc_broadleaf":"ป่าใบกว้าง","clc_conifer":"ป่าสน","clc_mixed_forest":"ป่าผสม","clc_natural_grass":"ทุ่งหญ้าธรรมชาติ","clc_moor":"ทุ่งหญ้าพรุ / ฮีธแลนด์","clc_sclerophyllous":"พืชพรรณ Sclerophyllous","clc_transitional":"พื้นที่เปลี่ยนผ่านระหว่างป่าและพุ่มไม้","clc_beach":"ชายหาด / เนินทราย / ทราย","clc_bare_rock":"หินโล่ง","clc_sparse_veg":"พื้นที่พืชพรรณเบาบาง","clc_burnt":"พื้นที่ไหม้","clc_glacier":"ธารน้ำแข็ง / หิมะถาวร","clc_inland_marsh":"หนองน้ำภายใน","clc_peatbog":"หนองพีต","clc_salt_marsh":"หนองน้ำเค็ม","clc_salines":"นาเกลือ","clc_intertidal":"เขตน้ำขึ้นน้ำลง","clc_river":"ลำน้ำ","clc_lake":"แหล่งน้ำ","clc_lagoon":"ทะเลสาบชายฝั่ง","clc_estuary":"ปากแม่น้ำ","clc_sea":"ทะเล / มหาสมุทร"},"bn":{"lulc_tree":"গাছপালা আচ্ছাদন / বন","lulc_shrubland":"গুল্মভূমি","lulc_grassland":"তৃণভূমি / চারণভূমি","lulc_cropland":"ফসলি জমি","lulc_urban":"নির্মিত / শহুরে এলাকা","lulc_sparse":"অনাবৃত / বিরল গাছপালা","lulc_snow":"তুষার / বরফ","lulc_water":"জলাশয়","lulc_wetland":"জলাভূমি / বিল","lulc_mangrove":"ম্যানগ্রোভ","lulc_moss":"শ্যাওলা / লাইকেন","modis_evergreen_needle":"চিরসবুজ সুচালো পাতার বন","modis_evergreen_broad":"চিরসবুজ চওড়া পাতার বন","modis_deciduous_needle":"পর্ণমোচী সুচালো পাতার বন","modis_deciduous_broad":"পর্ণমোচী চওড়া পাতার বন","modis_mixed_forest":"মিশ্র বন","modis_closed_shrub":"ঘন গুল্মভূমি","modis_open_shrub":"খোলা গুল্মভূমি","modis_woody_savanna":"কাষ্ঠল সাভানা","modis_savanna":"সাভানা","modis_grassland":"তৃণভূমি","modis_wetland":"স্থায়ী জলাভূমি","modis_cropland":"ফসলি জমি","modis_urban":"শহুরে ও নির্মিত এলাকা","modis_crop_natural":"ফসল / প্রাকৃতিক গাছপালার মোজাইক","modis_snow":"তুষার ও বরফ","modis_bare":"অনুর্বর বা বিরল উদ্ভিদযুক্ত","modis_water":"জলাশয়","clc_urban_cont":"নিরবচ্ছিন্ন নগর কাঠামো","clc_urban_disc":"বিচ্ছিন্ন নগর কাঠামো","clc_industrial":"শিল্প / বাণিজ্যিক","clc_road_rail":"সড়ক / রেল নেটওয়ার্ক","clc_port":"বন্দর এলাকা","clc_airport":"বিমানবন্দর","clc_mine":"খনিজ উত্তোলন স্থান","clc_dump":"আবর্জনা ফেলার স্থান","clc_construction":"নির্মাণ স্থান","clc_urban_green":"নগর সবুজ এলাকা","clc_sport":"খেলাধুলা / বিনোদন","clc_arable_dry":"অসেচিত চাষযোগ্য জমি","clc_arable_irr":"সেচযুক্ত চাষযোগ্য জমি","clc_rice":"ধান ক্ষেত","clc_vineyard":"আঙুর ক্ষেত","clc_fruit":"ফলের বাগান","clc_olive":"জলপাই বাগান","clc_pasture":"চারণভূমি","clc_annual_mosaic":"বার্ষিক/স্থায়ী ফসলের মোজাইক","clc_complex_farm":"জটিল চাষাবাদ ধরন","clc_agri_natural":"প্রাকৃতিক গাছপালাসহ কৃষি","clc_agri_forest":"কৃষি-বনায়ন এলাকা","clc_broadleaf":"চওড়া পাতার বন","clc_conifer":"সুচালো পাতার বন","clc_mixed_forest":"মিশ্র বন","clc_natural_grass":"প্রাকৃতিক তৃণভূমি","clc_moor":"মুর / হিথল্যান্ড","clc_sclerophyllous":"শক্ত-পাতার উদ্ভিদ","clc_transitional":"অন্তর্বর্তী বনভূমি-গুল্ম","clc_beach":"সৈকত / বালিয়াড়ি / বালু","clc_bare_rock":"অনাবৃত পাথর","clc_sparse_veg":"বিরল উদ্ভিদযুক্ত এলাকা","clc_burnt":"দগ্ধ এলাকা","clc_glacier":"হিমবাহ / স্থায়ী তুষার","clc_inland_marsh":"অভ্যন্তরীণ জলাভূমি","clc_peatbog":"পিট বগ","clc_salt_marsh":"লবণাক্ত জলাভূমি","clc_salines":"লবণক্ষেত্র","clc_intertidal":"জোয়ার-ভাটার সমতল","clc_river":"জলধারা","clc_lake":"জলাশয়","clc_lagoon":"উপকূলীয় উপহ্রদ","clc_estuary":"মোহনা","clc_sea":"সমুদ্র / মহাসাগর"},"vi":{"lulc_tree":"Độ che phủ cây / Rừng","lulc_shrubland":"Cây bụi","lulc_grassland":"Đồng cỏ","lulc_cropland":"Đất nông nghiệp","lulc_urban":"Đô thị / Xây dựng","lulc_sparse":"Đất trống / Thực vật thưa thớt","lulc_snow":"Tuyết / Băng","lulc_water":"Mặt nước","lulc_wetland":"Đất ngập nước / Đầm lầy","lulc_mangrove":"Rừng ngập mặn","lulc_moss":"Rêu / Địa y","modis_evergreen_needle":"Rừng lá kim thường xanh","modis_evergreen_broad":"Rừng lá rộng thường xanh","modis_deciduous_needle":"Rừng lá kim rụng lá","modis_deciduous_broad":"Rừng lá rộng rụng lá","modis_mixed_forest":"Rừng hỗn giao","modis_closed_shrub":"Cây bụi rậm","modis_open_shrub":"Cây bụi thưa","modis_woody_savanna":"Xavan cây gỗ","modis_savanna":"Xavan","modis_grassland":"Đồng cỏ","modis_wetland":"Đất ngập nước thường xuyên","modis_cropland":"Đất nông nghiệp","modis_urban":"Đô thị / Xây dựng","modis_crop_natural":"Khảm nông nghiệp-tự nhiên","modis_snow":"Tuyết và băng","modis_bare":"Đất trống hoặc thực vật thưa","modis_water":"Mặt nước","clc_urban_cont":"Khu đô thị liền kề","clc_urban_disc":"Khu đô thị rời rạc","clc_industrial":"Công nghiệp / Thương mại","clc_road_rail":"Mạng lưới đường bộ / đường sắt","clc_port":"Khu vực cảng","clc_airport":"Sân bay","clc_mine":"Khu vực khai thác mỏ","clc_dump":"Bãi rác","clc_construction":"Khu vực xây dựng","clc_urban_green":"Không gian xanh đô thị","clc_sport":"Cơ sở thể thao / Giải trí","clc_arable_dry":"Đất canh tác nhờ nước mưa","clc_arable_irr":"Đất canh tác có tưới","clc_rice":"Ruộng lúa","clc_vineyard":"Vườn nho","clc_fruit":"Vườn cây ăn quả","clc_olive":"Vườn ô liu","clc_pasture":"Đồng cỏ chăn thả","clc_annual_mosaic":"Khảm cây trồng hàng năm / lâu năm","clc_complex_farm":"Mô hình canh tác phức hợp","clc_agri_natural":"Nông nghiệp xen thực vật tự nhiên","clc_agri_forest":"Khu vực nông lâm kết hợp","clc_broadleaf":"Rừng lá rộng","clc_conifer":"Rừng lá kim","clc_mixed_forest":"Rừng hỗn giao","clc_natural_grass":"Đồng cỏ tự nhiên","clc_moor":"Cây bụi hoang / Đồng hoang","clc_sclerophyllous":"Thực vật lá cứng","clc_transitional":"Đất rừng-cây bụi chuyển tiếp","clc_beach":"Bãi biển / Cồn cát / Cát","clc_bare_rock":"Đá trơ trọi","clc_sparse_veg":"Thực vật thưa thớt","clc_burnt":"Khu vực bị cháy","clc_glacier":"Sông băng / Tuyết vĩnh cửu","clc_inland_marsh":"Đầm lầy nội địa","clc_peatbog":"Đầm lầy than bùn","clc_salt_marsh":"Đầm lầy nước mặn","clc_salines":"Ruộng muối","clc_intertidal":"Bãi triều","clc_river":"Sông / Dòng chảy","clc_lake":"Mặt nước","clc_lagoon":"Đầm phá ven biển","clc_estuary":"Cửa sông","clc_sea":"Biển / Đại dương"},"ur":{"lulc_tree":"درختوں کا ڈھکن / جنگل","lulc_shrubland":"جھاڑیاں","lulc_grassland":"گھاس کا میدان","lulc_cropland":"زرعی زمین","lulc_urban":"شہری / تعمیراتی","lulc_sparse":"ننگی / کم پودوں والی زمین","lulc_snow":"برف / گلیشیر","lulc_water":"آبی ذخیرہ","lulc_wetland":"دلدل / آبی زمین","lulc_mangrove":"مینگروو","lulc_moss":"کائی / لائیکن","modis_evergreen_needle":"سدابہار سوئی نما جنگل","modis_evergreen_broad":"سدابہار پتوں والا جنگل","modis_deciduous_needle":"پت جھڑ سوئی نما جنگل","modis_deciduous_broad":"پت جھڑ پتوں والا جنگل","modis_mixed_forest":"مخلوط جنگلات","modis_closed_shrub":"بند جھاڑیاں","modis_open_shrub":"کھلی جھاڑیاں","modis_woody_savanna":"لکڑی دار سوانا","modis_savanna":"سوانا","modis_grassland":"گھاس کا میدان","modis_wetland":"مستقل دلدل","modis_cropland":"زرعی زمین","modis_urban":"شہری / تعمیراتی","modis_crop_natural":"زرعی-قدرتی اختلاط","modis_snow":"برف اور گلیشیر","modis_bare":"ننگی زمین / کم پودے","modis_water":"آبی ذخیرہ","clc_urban_cont":"مسلسل شہری ڈھانچہ","clc_urban_disc":"منقطع شہری ڈھانچہ","clc_industrial":"صنعتی / تجارتی","clc_road_rail":"سڑک / ریل نیٹ ورک","clc_port":"بندرگاہ","clc_airport":"ہوائی اڈے","clc_mine":"کان کنی","clc_dump":"کچرے کا ڈھیر","clc_construction":"تعمیراتی مقامات","clc_urban_green":"شہری سبز جگہیں","clc_sport":"کھیل / تفریحی سہولیات","clc_arable_dry":"غیر آبپاش قابل کاشت","clc_arable_irr":"آبپاش قابل کاشت","clc_rice":"چاول کے کھیت","clc_vineyard":"انگور کے باغات","clc_fruit":"پھلوں کے باغات","clc_olive":"زیتون کے باغات","clc_pasture":"چراگاہیں","clc_annual_mosaic":"سالانہ / مستقل فصلیں (اختلاط)","clc_complex_farm":"پیچیدہ زرعی نظام","clc_agri_natural":"قدرتی پودوں کے ساتھ زراعت","clc_agri_forest":"زرعی-جنگلاتی زمین","clc_broadleaf":"چوڑے پتوں والے جنگلات","clc_conifer":"سوئی نما جنگلات","clc_mixed_forest":"مخلوط جنگلات","clc_natural_grass":"قدرتی گھاس کے میدان","clc_moor":"جھاڑ جھنکار","clc_sclerophyllous":"سخت پتوں والی پودے","clc_transitional":"جنگل / جھاڑیوں کا انتقال","clc_beach":"ساحل / ریت کے ٹیلے","clc_bare_rock":"ننگی چٹانیں","clc_sparse_veg":"کم پودے","clc_burnt":"جلے ہوئے علاقے","clc_glacier":"گلیشیر / مستقل برف","clc_inland_marsh":"اندرونی دلدل","clc_peatbog":"پیٹ بوگ","clc_salt_marsh":"نمکین دلدل","clc_salines":"نمکیات","clc_intertidal":"بین المدی کیچڑ","clc_river":"دریا","clc_lake":"جھیلیں","clc_lagoon":"ساحلی جھیلیں","clc_estuary":"دریا کا منہ","clc_sea":"سمندر / بحر"},"pl":{"lulc_tree":"Pokrywa drzewna / Las","lulc_shrubland":"Zarośla","lulc_grassland":"Łąki / Pastwiska","lulc_cropland":"Grunty orne","lulc_urban":"Zabudowa / Tereny miejskie","lulc_sparse":"Goła / Rzadka roślinność","lulc_snow":"Śnieg / Lód","lulc_water":"Zbiornik wodny","lulc_wetland":"Mokradło / Bagno","lulc_mangrove":"Namorzyny","lulc_moss":"Mchy / Porosty","modis_evergreen_needle":"Wiecznie zielony las iglasty","modis_evergreen_broad":"Wiecznie zielony las liściasty","modis_deciduous_needle":"Liściasty las iglasty","modis_deciduous_broad":"Liściasty las liściasty","modis_mixed_forest":"Lasy mieszane","modis_closed_shrub":"Zwarte zarośla","modis_open_shrub":"Otwarte zarośla","modis_woody_savanna":"Zalesiona sawanna","modis_savanna":"Sawanna","modis_grassland":"Łąki","modis_wetland":"Stałe mokradła","modis_cropland":"Grunty orne","modis_urban":"Tereny miejskie","modis_crop_natural":"Mozaika gruntów ornych i naturalnych","modis_snow":"Śnieg i lód","modis_bare":"Jałowe lub rzadko zarośnięte","modis_water":"Zbiorniki wodne","clc_urban_cont":"Ciągła zabudowa miejska","clc_urban_disc":"Nieciągła zabudowa miejska","clc_industrial":"Tereny przemysłowe / handlowe","clc_road_rail":"Drogi / Koleje","clc_port":"Tereny portowe","clc_airport":"Lotniska","clc_mine":"Tereny wydobywcze","clc_dump":"Wysypiska śmieci","clc_construction":"Place budowy","clc_urban_green":"Tereny zielone miejskie","clc_sport":"Tereny sportowo-rekreacyjne","clc_arable_dry":"Grunty orne nienawadniane","clc_arable_irr":"Grunty orne nawadniane","clc_rice":"Pola ryżowe","clc_vineyard":"Winnice","clc_fruit":"Drzewa owocowe","clc_olive":"Gaje oliwne","clc_pasture":"Pastwiska","clc_annual_mosaic":"Mozaika upraw jednorocznych i wieloletnich","clc_complex_farm":"Złożone systemy upraw","clc_agri_natural":"Rolnictwo z naturalną roślinnością","clc_agri_forest":"Tereny rolno-leśne","clc_broadleaf":"Las liściasty","clc_conifer":"Las iglasty","clc_mixed_forest":"Las mieszany","clc_natural_grass":"Naturalne łąki","clc_moor":"Wrzosowiska / Moczary","clc_sclerophyllous":"Roślinność sklerofityczna","clc_transitional":"Lasy i zarośla w stanie przemian","clc_beach":"Plaże / Wydmy / Piaski","clc_bare_rock":"Gołe skały","clc_sparse_veg":"Rzadko zarośnięte obszary","clc_burnt":"Tereny spalone","clc_glacier":"Lodowce / Wieczny śnieg","clc_inland_marsh":"Bagna śródlądowe","clc_peatbog":"Torfowiska","clc_salt_marsh":"Słonawy nadmorskie","clc_salines":"Solniska","clc_intertidal":"Strefy pływowe","clc_river":"Cieki wodne","clc_lake":"Zbiorniki wodne","clc_lagoon":"Laguny przybrzeżne","clc_estuary":"Estuaria","clc_sea":"Morze / Ocean"},"ms":{"lulc_tree":"Litupan Pokok / Hutan","lulc_shrubland":"Semak Samun","lulc_grassland":"Padang Rumput / Ragut","lulc_cropland":"Tanah Pertanian","lulc_urban":"Kawasan Bangunan / Bandar","lulc_sparse":"Tanah Kosong / Tumbuhan Jarang","lulc_snow":"Salji / Ais","lulc_water":"Badan Air","lulc_wetland":"Tanah Lembap / Paya","lulc_mangrove":"Bakau","lulc_moss":"Lumut / Liken","modis_evergreen_needle":"Hutan Daun Jarum Sentiasa Hijau","modis_evergreen_broad":"Hutan Daun Lebar Sentiasa Hijau","modis_deciduous_needle":"Hutan Daun Jarum Luruh","modis_deciduous_broad":"Hutan Daun Lebar Luruh","modis_mixed_forest":"Hutan Campuran","modis_closed_shrub":"Semak Tertutup","modis_open_shrub":"Semak Terbuka","modis_woody_savanna":"Savana Berkayu","modis_savanna":"Savana","modis_grassland":"Padang Rumput","modis_wetland":"Tanah Lembap Kekal","modis_cropland":"Tanah Pertanian","modis_urban":"Bandar dan Kawasan Bangunan","modis_crop_natural":"Mozek Tanah Pertanian / Tumbuhan Semula Jadi","modis_snow":"Salji dan Ais","modis_bare":"Tandus atau Tumbuhan Jarang","modis_water":"Badan Air","clc_urban_cont":"Struktur Bandar Berterusan","clc_urban_disc":"Struktur Bandar Tidak Berterusan","clc_industrial":"Perindustrian / Perdagangan","clc_road_rail":"Rangkaian Jalan Raya / Rel","clc_port":"Kawasan Pelabuhan","clc_airport":"Lapangan Terbang","clc_mine":"Tapak Perlombongan","clc_dump":"Tapak Pelupusan","clc_construction":"Tapak Pembinaan","clc_urban_green":"Kawasan Hijau Bandar","clc_sport":"Sukan / Rekreasi","clc_arable_dry":"Tanah Tanaman Tanpa Pengairan","clc_arable_irr":"Tanah Tanaman Beririgasi","clc_rice":"Sawah Padi","clc_vineyard":"Ladang Anggur","clc_fruit":"Pokok Buah-buahan","clc_olive":"Ladang Zaitun","clc_pasture":"Padang Ragut","clc_annual_mosaic":"Mozek Tanaman Tahunan / Kekal","clc_complex_farm":"Corak Penanaman Kompleks","clc_agri_natural":"Pertanian dengan Tumbuhan Semula Jadi","clc_agri_forest":"Kawasan Agroforestri","clc_broadleaf":"Hutan Daun Lebar","clc_conifer":"Hutan Konifer","clc_mixed_forest":"Hutan Campuran","clc_natural_grass":"Padang Rumput Semula Jadi","clc_moor":"Tanah Lapang / Paya Renek","clc_sclerophyllous":"Tumbuhan Sklerofil","clc_transitional":"Hutan-Semak Peralihan","clc_beach":"Pantai / Bukit Pasir / Pasir","clc_bare_rock":"Batu Kosong","clc_sparse_veg":"Kawasan Tumbuhan Jarang","clc_burnt":"Kawasan Terbakar","clc_glacier":"Glasier / Salji Berkekalan","clc_inland_marsh":"Paya Pedalaman","clc_peatbog":"Tanah Gambut","clc_salt_marsh":"Paya Garam","clc_salines":"Kawasan Masin","clc_intertidal":"Kawasan Intertidal","clc_river":"Aliran Air","clc_lake":"Badan Air","clc_lagoon":"Lagun Pantai","clc_estuary":"Muara","clc_sea":"Laut / Lautan"},"ky":{"lulc_tree":"Дарак жамынышы / Токой","lulc_shrubland":"Бадалдар","lulc_grassland":"Чалгын / Жайлоо","lulc_cropland":"Айдалган жер","lulc_urban":"Шаардык / Куруулуу аймак","lulc_sparse":"Жалаңач / Сейрек өсүмдүк","lulc_snow":"Кар / Муз","lulc_water":"Суу беткейи","lulc_wetland":"Батпак / Сулуу жер","lulc_mangrove":"Мангр","lulc_moss":"Мох / Лишайник","modis_evergreen_needle":"Мазмаз жашыл ийнелүү токой","modis_evergreen_broad":"Мазмаз жашыл кең жалбырактуу токой","modis_deciduous_needle":"Жалбырак төгүүчү ийнелүү токой","modis_deciduous_broad":"Жалбырак төгүүчү кең жалбырактуу токой","modis_mixed_forest":"Аралаш токой","modis_closed_shrub":"Жабык бадал","modis_open_shrub":"Ачык бадал","modis_woody_savanna":"Дарактуу саванна","modis_savanna":"Саванна","modis_grassland":"Чалгын","modis_wetland":"Туруктуу батпак","modis_cropland":"Айдалган жер","modis_urban":"Шаардык / Куруулуу аймак","modis_crop_natural":"Айыл чарба–табигый мозаика","modis_snow":"Кар жана муз","modis_bare":"Жалаңач жер / Сейрек жамыныш","modis_water":"Суу беткейи","clc_urban_cont":"Үзгүлтүксүз шаардык аймак","clc_urban_disc":"Үзгүлтүктүү шаардык аймак","clc_industrial":"Өнөр жай / Соода","clc_road_rail":"Жол / Темир жол тармагы","clc_port":"Порт аймактары","clc_airport":"Аэропорттор","clc_mine":"Кен аймактары","clc_dump":"Таштанды полигондору","clc_construction":"Курулуш аймактары","clc_urban_green":"Шаардык жашыл аймактар","clc_sport":"Спорт / Эс алуу","clc_arable_dry":"Суарылбаган айдалган жер","clc_arable_irr":"Суарылган айдалган жер","clc_rice":"Күрүч талаалары","clc_vineyard":"Жүзүм бактары","clc_fruit":"Жемиш бактары","clc_olive":"Зайтун бактары","clc_pasture":"Жайыттар","clc_annual_mosaic":"Жылдык / Көп жылдык мозаика","clc_complex_farm":"Татаал дыйканчылык системасы","clc_agri_natural":"Табигый өсүмдүктөр менен айыл чарба","clc_agri_forest":"Айыл чарба–токой аймагы","clc_broadleaf":"Кең жалбырактуу токой","clc_conifer":"Ийнелүү токой","clc_mixed_forest":"Аралаш токой","clc_natural_grass":"Табигый чалгын","clc_moor":"Дала / Бадалдар","clc_sclerophyllous":"Катуу жалбырактуу өсүмдүк","clc_transitional":"Токой–бадал өтүү аймагы","clc_beach":"Жээк / Кум / Дюна","clc_bare_rock":"Жалаңач аска","clc_sparse_veg":"Сейрек өсүмдүк аймагы","clc_burnt":"Өрттөнгөн аймак","clc_glacier":"Мөңгү / Туруктуу кар","clc_inland_marsh":"Ички батпак","clc_peatbog":"Торфтуу батпак","clc_salt_marsh":"Туздуу батпак","clc_salines":"Туздуу аймактар","clc_intertidal":"Аккалкан аймак","clc_river":"Дарыя / Агын","clc_lake":"Көл","clc_lagoon":"Жээк лагунасы","clc_estuary":"Дарыя оозу","clc_sea":"Деңиз / Мухит"},"mn":{"lulc_tree":"Модны бүрхэвч / Ой","lulc_shrubland":"Бут сөөг","lulc_grassland":"Бэлчээр / Нуга","lulc_cropland":"Тариалангийн газар","lulc_urban":"Суурин / Хот","lulc_sparse":"Нүцгэн / Сийрэг ургамал","lulc_snow":"Цас / Мөс","lulc_water":"Усны гадаргуу","lulc_wetland":"Намаг / Шавартай газар","lulc_mangrove":"Мангров","lulc_moss":"Хаг / Ягаан","modis_evergreen_needle":"Мөнх ногоон зүү навчит ой","modis_evergreen_broad":"Мөнх ногоон өргөн навчит ой","modis_deciduous_needle":"Навч унагадаг зүү навчит ой","modis_deciduous_broad":"Навч унагадаг өргөн навчит ой","modis_mixed_forest":"Холимог ой","modis_closed_shrub":"Нягт бут сөөг","modis_open_shrub":"Нээлттэй бут сөөг","modis_woody_savanna":"Модлог саванна","modis_savanna":"Саванна","modis_grassland":"Бэлчээр","modis_wetland":"Байнгын намаг","modis_cropland":"Тариалангийн газар","modis_urban":"Хот ба суурин","modis_crop_natural":"Тариалан / Байгалийн ургамлын мозайк","modis_snow":"Цас ба мөс","modis_bare":"Нүцгэн / Сийрэг ургамалтай газар","modis_water":"Усны биет","clc_urban_cont":"Тасралтгүй хотын суурин","clc_urban_disc":"Тасалдалтай хотын суурин","clc_industrial":"Үйлдвэрлэл / Худалдаа","clc_road_rail":"Зам / Төмөр зам","clc_port":"Боомтын бүс","clc_airport":"Нисэх буудлууд","clc_mine":"Уул уурхайн бүс","clc_dump":"Хог хаягдлын цэг","clc_construction":"Барилгын бүс","clc_urban_green":"Хотын ногоон бүс","clc_sport":"Спорт / Амралт","clc_arable_dry":"Усалгаагүй тариалангийн газар","clc_arable_irr":"Усалгаатай тариалангийн газар","clc_rice":"Будааны тариалан","clc_vineyard":"Усан үзмийн тариалан","clc_fruit":"Жимсний мод","clc_olive":"Чидун жимсний ой","clc_pasture":"Бэлчээр","clc_annual_mosaic":"Жилийн / Олон жилийн тариаланы мозайк","clc_complex_farm":"Нарийн ажиллагааны хэв маяг","clc_agri_natural":"Байгалийн ургамалтай хамт хөдөө аж ахуй","clc_agri_forest":"Хөдөө аж ахуй-ойн бүс","clc_broadleaf":"Өргөн навчит ой","clc_conifer":"Шилмүүст ой","clc_mixed_forest":"Холимог ой","clc_natural_grass":"Байгалийн бэлчээр","clc_moor":"Нуурын дэнж / Хяр","clc_sclerophyllous":"Хатуу навчит ургамал","clc_transitional":"Ой-бут сөөгийн шилжилтийн бүс","clc_beach":"Эрэг / Элсэн манхан / Элс","clc_bare_rock":"Нүцгэн чулуу","clc_sparse_veg":"Сийрэг ургамалтай бүс","clc_burnt":"Шатсан бүс","clc_glacier":"Мөсөн гол / Байнгын цас","clc_inland_marsh":"Дотоодын намаг","clc_peatbog":"Хүлэрт намаг","clc_salt_marsh":"Давстай намаг","clc_salines":"Давстай бүс","clc_intertidal":"Далайн татлагын бүс","clc_river":"Усны урсгал","clc_lake":"Усны биет","clc_lagoon":"Эрэгт нуур","clc_estuary":"Голын амны нуур","clc_sea":"Далай / Тэнгис"},"ka":{"lulc_tree":"ხის საფარი / ტყე","lulc_shrubland":"ბუჩქნარი","lulc_grassland":"მდელო / საძოვარი","lulc_cropland":"სახნავი მიწა","lulc_urban":"ურბანული / განაშენიანებული ტერიტორია","lulc_sparse":"შიშველი / იშვიათი მცენარეობა","lulc_snow":"თოვლი / ყინული","lulc_water":"წყლის ზედაპირი","lulc_wetland":"ჭაობი / სველი ადგილი","lulc_mangrove":"მანგრო","lulc_moss":"ხავსი / ქერქი","modis_evergreen_needle":"მარადმწვანე წიწვოვანი ტყე","modis_evergreen_broad":"მარადმწვანე ფართოფოთლოვანი ტყე","modis_deciduous_needle":"ფოთოლმცვენი წიწვოვანი ტყე","modis_deciduous_broad":"ფოთოლმცვენი ფართოფოთლოვანი ტყე","modis_mixed_forest":"შერეული ტყე","modis_closed_shrub":"დახურული ბუჩქნარი","modis_open_shrub":"ღია ბუჩქნარი","modis_woody_savanna":"ხეიანი სავანა","modis_savanna":"სავანა","modis_grassland":"მდელო","modis_wetland":"მუდმივი ჭაობი","modis_cropland":"სახნავი მიწა","modis_urban":"ურბანული / განაშენიანებული ტერიტორია","modis_crop_natural":"სასოფლო-სამეურნეო–ბუნებრივი მოზაიკა","modis_snow":"თოვლი და ყინული","modis_bare":"შიშველი მიწა / იშვიათი საფარი","modis_water":"წყლის ზედაპირი","clc_urban_cont":"უწყვეტი ურბანული ქსოვილი","clc_urban_disc":"დაუწყვეტელი ურბანული ქსოვილი","clc_industrial":"სამრეწველო / სავაჭრო","clc_road_rail":"გზა / სარკინიგზო ქსელი","clc_port":"სანავსადგურო ტერიტორიები","clc_airport":"აეროპორტები","clc_mine":"სამთო ტერიტორიები","clc_dump":"ნაგვის პოლიგონები","clc_construction":"სამშენებლო ტერიტორიები","clc_urban_green":"ქალაქის სასარგებლო მწვანე ზონები","clc_sport":"სპორტი / დასვენება","clc_arable_dry":"სარწყავი სახნავი","clc_arable_irr":"სარწყავი სახნავი (მოსარწყავი)","clc_rice":"ბრინჯის მინდვრები","clc_vineyard":"ვენახები","clc_fruit":"ხილის ბაღები","clc_olive":"ზეითუნის ბაღები","clc_pasture":"საძოვრები","clc_annual_mosaic":"ყოველწლიური / მრავალწლიური მოზაიკა","clc_complex_farm":"რთული სასოფლო-სამეურნეო სისტემა","clc_agri_natural":"სასოფლო-სამეურნეო სფეროს ბუნებრივი მცენარეობა","clc_agri_forest":"სასოფლო-სამეურნეო-ტყის ტერიტორია","clc_broadleaf":"ფართოფოთლოვანი ტყე","clc_conifer":"წიწვოვანი ტყე","clc_mixed_forest":"შერეული ტყე","clc_natural_grass":"ბუნებრივი მდელოები","clc_moor":"ველი / ბუჩქნარი","clc_sclerophyllous":"მყარფოთლოვანი მცენარეობა","clc_transitional":"ტყე-ბუჩქნარის გარდამავალი ზონა","clc_beach":"სანაპირო / ქვიშა / დიუნა","clc_bare_rock":"შიშველი კლდე","clc_sparse_veg":"იშვიათი მცენარეობის ზონა","clc_burnt":"დაწვილი ტერიტორია","clc_glacier":"მყინვარი / მუდმივი თოვლი","clc_inland_marsh":"შიდა ჭაობი","clc_peatbog":"ტორფის ჭაობი","clc_salt_marsh":"მარილიანი ჭაობი","clc_salines":"მარილიანი ტერიტორიები","clc_intertidal":"ინტერტიდალური ზონა","clc_river":"მდინარე / ნაკადი","clc_lake":"ტბა","clc_lagoon":"სანაპირო ლაგუნა","clc_estuary":"მდინარის შესართავი","clc_sea":"ზღვა / ოკეანე"},"it":{"lulc_tree":"Copertura Arborea / Foresta","lulc_shrubland":"Macchia","lulc_grassland":"Prato / Pascolo","lulc_cropland":"Terreno Agricolo","lulc_urban":"Edificato / Urbano","lulc_sparse":"Suolo Nudo / Vegetazione Rada","lulc_snow":"Neve / Ghiaccio","lulc_water":"Corpo Idrico","lulc_wetland":"Zone Umide / Paludi","lulc_mangrove":"Mangrovia","lulc_moss":"Muschio / Lichene","modis_evergreen_needle":"Foresta Sempreverde ad Aghifoglie","modis_evergreen_broad":"Foresta Sempreverde a Latifoglie","modis_deciduous_needle":"Foresta Decidua ad Aghifoglie","modis_deciduous_broad":"Foresta Decidua a Latifoglie","modis_mixed_forest":"Foreste Miste","modis_closed_shrub":"Macchia Chiusa","modis_open_shrub":"Macchia Aperta","modis_woody_savanna":"Savana Arborea","modis_savanna":"Savana","modis_grassland":"Prateria","modis_wetland":"Zone Umide Permanenti","modis_cropland":"Terreni Agricoli","modis_urban":"Urbano e Edificato","modis_crop_natural":"Mosaico Agricoltura / Vegetazione Naturale","modis_snow":"Neve e Ghiaccio","modis_bare":"Suolo Nudo o Scarsamente Vegetato","modis_water":"Corpi Idrici","clc_urban_cont":"Tessuto Urbano Continuo","clc_urban_disc":"Tessuto Urbano Discontinuo","clc_industrial":"Industriale / Commerciale","clc_road_rail":"Reti Stradali / Ferroviarie","clc_port":"Aree Portuali","clc_airport":"Aeroporti","clc_mine":"Siti di Estrazione Mineraria","clc_dump":"Discariche","clc_construction":"Cantieri","clc_urban_green":"Aree Verdi Urbane","clc_sport":"Sport / Tempo Libero","clc_arable_dry":"Seminativi Non Irrigui","clc_arable_irr":"Seminativi Irrigui","clc_rice":"Risaie","clc_vineyard":"Vigneti","clc_fruit":"Frutteti","clc_olive":"Oliveti","clc_pasture":"Pascoli","clc_annual_mosaic":"Mosaico Colture Annuali / Permanenti","clc_complex_farm":"Sistemi Colturali Complessi","clc_agri_natural":"Agricoltura con Vegetazione Naturale","clc_agri_forest":"Agroforestale","clc_broadleaf":"Bosco di Latifoglie","clc_conifer":"Bosco di Conifere","clc_mixed_forest":"Bosco Misto","clc_natural_grass":"Praterie Naturali","clc_moor":"Brughiere / Lande","clc_sclerophyllous":"Vegetazione Sclerofilla","clc_transitional":"Aree di Transizione Bosco-Cespuglio","clc_beach":"Spiagge / Dune / Sabbie","clc_bare_rock":"Roccia Nuda","clc_sparse_veg":"Aree con Scarsa Vegetazione","clc_burnt":"Aree Bruciate","clc_glacier":"Ghiacciai / Neve Permanente","clc_inland_marsh":"Paludi Interne","clc_peatbog":"Torbiere","clc_salt_marsh":"Paludi Salmastre","clc_salines":"Saline","clc_intertidal":"Aree Intertidali","clc_river":"Corsi d'Acqua","clc_lake":"Specchi d'Acqua","clc_lagoon":"Lagune Costiere","clc_estuary":"Estuari","clc_sea":"Mare / Oceano"}}
_ASPECT_LABELS_30={"en":{"flat":"Flat (-1)","north":"North (0–12.5° / 327.5–360°)","northeast":"Northeast (12.5–57.5°)","east":"East (57.5–102.5°)","southeast":"Southeast (102.5–147.5°)","south":"South (147.5–192.5°)","southwest":"Southwest (192.5–237.5°)","west":"West (237.5–282.5°)","northwest":"Northwest (282.5–327.5°)"},"uz":{"flat":"Tekis (-1)","north":"Shimol (0–12.5° / 327.5–360°)","northeast":"Shimoli-sharq (12.5–57.5°)","east":"Sharq (57.5–102.5°)","southeast":"Janubi-sharq (102.5–147.5°)","south":"Janub (147.5–192.5°)","southwest":"Janubi-g‘arb (192.5–237.5°)","west":"G‘arb (237.5–282.5°)","northwest":"Shimoli-g‘arb (282.5–327.5°)"},"el":{"flat":"Επίπεδο (-1)","north":"Βορράς (0–12.5° / 327.5–360°)","northeast":"Βορειοανατολικά (12.5–57.5°)","east":"Ανατολή (57.5–102.5°)","southeast":"Νοτιοανατολικά (102.5–147.5°)","south":"Νότος (147.5–192.5°)","southwest":"Νοτιοδυτικά (192.5–237.5°)","west":"Δύση (237.5–282.5°)","northwest":"Βορειοδυτικά (282.5–327.5°)"},"bg":{"flat":"Равен (-1)","north":"Север (0–12.5° / 327.5–360°)","northeast":"Североизток (12.5–57.5°)","east":"Изток (57.5–102.5°)","southeast":"Югоизток (102.5–147.5°)","south":"Юг (147.5–192.5°)","southwest":"Югозапад (192.5–237.5°)","west":"Запад (237.5–282.5°)","northwest":"Северозапад (282.5–327.5°)"},"tr":{"flat":"Düz (-1)","north":"Kuzey (0–12.5° / 327.5–360°)","northeast":"Kuzeydoğu (12.5–57.5°)","east":"Doğu (57.5–102.5°)","southeast":"Güneydoğu (102.5–147.5°)","south":"Güney (147.5–192.5°)","southwest":"Güneybatı (192.5–237.5°)","west":"Batı (237.5–282.5°)","northwest":"Kuzeybatı (282.5–327.5°)"},"az":{"flat":"Düz (-1)","north":"Şimal (0–12.5° / 327.5–360°)","northeast":"Şimal-şərq (12.5–57.5°)","east":"Şərq (57.5–102.5°)","southeast":"Cənub-şərq (102.5–147.5°)","south":"Cənub (147.5–192.5°)","southwest":"Cənub-qərb (192.5–237.5°)","west":"Qərb (237.5–282.5°)","northwest":"Şimal-qərb (282.5–327.5°)"},"hu":{"flat":"Sík (-1)","north":"Észak (0–12.5° / 327.5–360°)","northeast":"Északkelet (12.5–57.5°)","east":"Kelet (57.5–102.5°)","southeast":"Délkelet (102.5–147.5°)","south":"Dél (147.5–192.5°)","southwest":"Délnyugat (192.5–237.5°)","west":"Nyugat (237.5–282.5°)","northwest":"Északnyugat (282.5–327.5°)"},"kk":{"flat":"Жазық (-1)","north":"Солтүстік (0–12.5° / 327.5–360°)","northeast":"Солтүстік-шығыс (12.5–57.5°)","east":"Шығыс (57.5–102.5°)","southeast":"Оңтүстік-шығыс (102.5–147.5°)","south":"Оңтүстік (147.5–192.5°)","southwest":"Оңтүстік-батыс (192.5–237.5°)","west":"Батыс (237.5–282.5°)","northwest":"Солтүстік-батыс (282.5–327.5°)"},"de":{"flat":"Flach (-1)","north":"Nord (0–12.5° / 327.5–360°)","northeast":"Nordost (12.5–57.5°)","east":"Ost (57.5–102.5°)","southeast":"Südost (102.5–147.5°)","south":"Süd (147.5–192.5°)","southwest":"Südwest (192.5–237.5°)","west":"West (237.5–282.5°)","northwest":"Nordwest (282.5–327.5°)"},"fr":{"flat":"Plat (-1)","north":"Nord (0–12.5° / 327.5–360°)","northeast":"Nord-est (12.5–57.5°)","east":"Est (57.5–102.5°)","southeast":"Sud-est (102.5–147.5°)","south":"Sud (147.5–192.5°)","southwest":"Sud-ouest (192.5–237.5°)","west":"Ouest (237.5–282.5°)","northwest":"Nord-ouest (282.5–327.5°)"},"es":{"flat":"Plano (-1)","north":"Norte (0–12.5° / 327.5–360°)","northeast":"Noreste (12.5–57.5°)","east":"Este (57.5–102.5°)","southeast":"Sureste (102.5–147.5°)","south":"Sur (147.5–192.5°)","southwest":"Suroeste (192.5–237.5°)","west":"Oeste (237.5–282.5°)","northwest":"Noroeste (282.5–327.5°)"},"pt":{"flat":"Plano (-1)","north":"Norte (0–12.5° / 327.5–360°)","northeast":"Nordeste (12.5–57.5°)","east":"Leste (57.5–102.5°)","southeast":"Sudeste (102.5–147.5°)","south":"Sul (147.5–192.5°)","southwest":"Sudoeste (192.5–237.5°)","west":"Oeste (237.5–282.5°)","northwest":"Noroeste (282.5–327.5°)"},"ar":{"flat":"مستوٍ (-1)","north":"الشمال (0–12.5° / 327.5–360°)","northeast":"الشمال الشرقي (12.5–57.5°)","east":"الشرق (57.5–102.5°)","southeast":"الجنوب الشرقي (102.5–147.5°)","south":"الجنوب (147.5–192.5°)","southwest":"الجنوب الغربي (192.5–237.5°)","west":"الغرب (237.5–282.5°)","northwest":"الشمال الغربي (282.5–327.5°)"},"fa":{"flat":"مسطح (-1)","north":"شمال (0–12.5° / 327.5–360°)","northeast":"شمال‌شرق (12.5–57.5°)","east":"شرق (57.5–102.5°)","southeast":"جنوب‌شرق (102.5–147.5°)","south":"جنوب (147.5–192.5°)","southwest":"جنوب‌غرب (192.5–237.5°)","west":"غرب (237.5–282.5°)","northwest":"شمال‌غرب (282.5–327.5°)"},"zh":{"flat":"平坦 (-1)","north":"北 (0–12.5° / 327.5–360°)","northeast":"东北 (12.5–57.5°)","east":"东 (57.5–102.5°)","southeast":"东南 (102.5–147.5°)","south":"南 (147.5–192.5°)","southwest":"西南 (192.5–237.5°)","west":"西 (237.5–282.5°)","northwest":"西北 (282.5–327.5°)"},"ja":{"flat":"平坦 (-1)","north":"北 (0–12.5° / 327.5–360°)","northeast":"北東 (12.5–57.5°)","east":"東 (57.5–102.5°)","southeast":"南東 (102.5–147.5°)","south":"南 (147.5–192.5°)","southwest":"南西 (192.5–237.5°)","west":"西 (237.5–282.5°)","northwest":"北西 (282.5–327.5°)"},"ko":{"flat":"평탄 (-1)","north":"북 (0–12.5° / 327.5–360°)","northeast":"북동 (12.5–57.5°)","east":"동 (57.5–102.5°)","southeast":"남동 (102.5–147.5°)","south":"남 (147.5–192.5°)","southwest":"남서 (192.5–237.5°)","west":"서 (237.5–282.5°)","northwest":"북서 (282.5–327.5°)"},"hi":{"flat":"समतल (-1)","north":"उत्तर (0–12.5° / 327.5–360°)","northeast":"उत्तर-पूर्व (12.5–57.5°)","east":"पूर्व (57.5–102.5°)","southeast":"दक्षिण-पूर्व (102.5–147.5°)","south":"दक्षिण (147.5–192.5°)","southwest":"दक्षिण-पश्चिम (192.5–237.5°)","west":"पश्चिम (237.5–282.5°)","northwest":"उत्तर-पश्चिम (282.5–327.5°)"},"id":{"flat":"Datar (-1)","north":"Utara (0–12.5° / 327.5–360°)","northeast":"Timur Laut (12.5–57.5°)","east":"Timur (57.5–102.5°)","southeast":"Tenggara (102.5–147.5°)","south":"Selatan (147.5–192.5°)","southwest":"Barat Daya (192.5–237.5°)","west":"Barat (237.5–282.5°)","northwest":"Barat Laut (282.5–327.5°)"},"ru":{"flat":"Ровный (-1)","north":"Север (0–12.5° / 327.5–360°)","northeast":"Северо-восток (12.5–57.5°)","east":"Восток (57.5–102.5°)","southeast":"Юго-восток (102.5–147.5°)","south":"Юг (147.5–192.5°)","southwest":"Юго-запад (192.5–237.5°)","west":"Запад (237.5–282.5°)","northwest":"Северо-запад (282.5–327.5°)"},"th":{"flat":"ราบ (-1)","north":"เหนือ (0–12.5° / 327.5–360°)","northeast":"ตะวันออกเฉียงเหนือ (12.5–57.5°)","east":"ตะวันออก (57.5–102.5°)","southeast":"ตะวันออกเฉียงใต้ (102.5–147.5°)","south":"ใต้ (147.5–192.5°)","southwest":"ตะวันตกเฉียงใต้ (192.5–237.5°)","west":"ตะวันตก (237.5–282.5°)","northwest":"ตะวันตกเฉียงเหนือ (282.5–327.5°)"},"bn":{"flat":"সমতল (-1)","north":"উত্তর (0–12.5° / 327.5–360°)","northeast":"উত্তর-পূর্ব (12.5–57.5°)","east":"পূর্ব (57.5–102.5°)","southeast":"দক্ষিণ-পূর্ব (102.5–147.5°)","south":"দক্ষিণ (147.5–192.5°)","southwest":"দক্ষিণ-পশ্চিম (192.5–237.5°)","west":"পশ্চিম (237.5–282.5°)","northwest":"উত্তর-পশ্চিম (282.5–327.5°)"},"vi":{"flat":"Bằng phẳng (-1)","north":"Bắc (0–12.5° / 327.5–360°)","northeast":"Đông Bắc (12.5–57.5°)","east":"Đông (57.5–102.5°)","southeast":"Đông Nam (102.5–147.5°)","south":"Nam (147.5–192.5°)","southwest":"Tây Nam (192.5–237.5°)","west":"Tây (237.5–282.5°)","northwest":"Tây Bắc (282.5–327.5°)"},"ur":{"flat":"ہموار (-1)","north":"شمال (0–12.5° / 327.5–360°)","northeast":"شمال مشرق (12.5–57.5°)","east":"مشرق (57.5–102.5°)","southeast":"جنوب مشرق (102.5–147.5°)","south":"جنوب (147.5–192.5°)","southwest":"جنوب مغرب (192.5–237.5°)","west":"مغرب (237.5–282.5°)","northwest":"شمال مغرب (282.5–327.5°)"},"pl":{"flat":"Płaski (-1)","north":"Północ (0–12.5° / 327.5–360°)","northeast":"Północny wschód (12.5–57.5°)","east":"Wschód (57.5–102.5°)","southeast":"Południowy wschód (102.5–147.5°)","south":"Południe (147.5–192.5°)","southwest":"Południowy zachód (192.5–237.5°)","west":"Zachód (237.5–282.5°)","northwest":"Północny zachód (282.5–327.5°)"},"ms":{"flat":"Rata (-1)","north":"Utara (0–12.5° / 327.5–360°)","northeast":"Timur Laut (12.5–57.5°)","east":"Timur (57.5–102.5°)","southeast":"Tenggara (102.5–147.5°)","south":"Selatan (147.5–192.5°)","southwest":"Barat Daya (192.5–237.5°)","west":"Barat (237.5–282.5°)","northwest":"Barat Laut (282.5–327.5°)"},"ky":{"flat":"Түз (-1)","north":"Түндүк (0–12.5° / 327.5–360°)","northeast":"Түндүк-чыгыш (12.5–57.5°)","east":"Чыгыш (57.5–102.5°)","southeast":"Түштүк-чыгыш (102.5–147.5°)","south":"Түштүк (147.5–192.5°)","southwest":"Түштүк-батыш (192.5–237.5°)","west":"Батыш (237.5–282.5°)","northwest":"Түндүк-батыш (282.5–327.5°)"},"mn":{"flat":"Тэгш (-1)","north":"Хойд (0–12.5° / 327.5–360°)","northeast":"Зүүн хойд (12.5–57.5°)","east":"Зүүн (57.5–102.5°)","southeast":"Зүүн өмнөд (102.5–147.5°)","south":"Өмнөд (147.5–192.5°)","southwest":"Баруун өмнөд (192.5–237.5°)","west":"Баруун (237.5–282.5°)","northwest":"Баруун хойд (282.5–327.5°)"},"ka":{"flat":"ბრტყელი (-1)","north":"ჩრდილოეთი (0–12.5° / 327.5–360°)","northeast":"ჩრდილო-აღმოსავლეთი (12.5–57.5°)","east":"აღმოსავლეთი (57.5–102.5°)","southeast":"სამხრეთ-აღმოსავლეთი (102.5–147.5°)","south":"სამხრეთი (147.5–192.5°)","southwest":"სამხრეთ-დასავლეთი (192.5–237.5°)","west":"დასავლეთი (237.5–282.5°)","northwest":"ჩრდილო-დასავლეთი (282.5–327.5°)"},"it":{"flat":"Pianeggiante (-1)","north":"Nord (0–12.5° / 327.5–360°)","northeast":"Nord-est (12.5–57.5°)","east":"Est (57.5–102.5°)","southeast":"Sud-est (102.5–147.5°)","south":"Sud (147.5–192.5°)","southwest":"Sud-ovest (192.5–237.5°)","west":"Ovest (237.5–282.5°)","northwest":"Nord-ovest (282.5–327.5°)"}}
def _request_lang(data):
    lg=str((data or {}).get('lang') or '').lower().replace('_','-').split('-')[0]
    if lg in _RASTER_CLASS_LABELS_30: return lg
    try:
        for p in request.headers.get('Accept-Language','').split(','):
            q=p.strip().split(';')[0].lower().split('-')[0]
            if q in _RASTER_CLASS_LABELS_30: return q
    except Exception: pass
    return 'en'
def _localized_raster_label(lang,key,fallback):
    return _RASTER_CLASS_LABELS_30.get(lang,{}).get(key) or _RASTER_CLASS_LABELS_30['en'].get(key) or fallback
def _aspect_code_info(lang):
    """Aspect 9 sınıfı için kod -> (yerelleştirilmiş ad, RGB) sözlüğü.

    Bu yardımcı daha önce yanlışlıkla ZIP dosyası üretim değişkenlerine
    (`code_info`, `safe_name`, `new_tif_bytes` vb.) referans veriyordu. Bu
    nedenle Aspect GeoTIFF sembolojisinin dil değişiminde/indirmede güvenilir
    biçimde kurulması mümkün değildi. Artık yalnızca gerçekten ihtiyaç
    duyduğu 9 sınıf bilgisini döndürür; ZIP üretimini üst seviye fonksiyonlar
    yapar.
    """
    colors = [
    ...
    words = _ASPECT_LABELS_30.get(lang, _ASPECT_LABELS_30['en'])
    keys = ['flat','north','northeast','east','southeast','south','southwest','west','northwest']
    return {i + 1: (words.get(k, _ASPECT_LABELS_30['en'][k]), colors[i])
            for i, k in enumerate(keys)}

def _build_lulc_symbology_zip(tif_bytes, index_name, safe_name, lang='en'):
    """
    ...
    for d in defs:
        ...
        code_info[d['code']] = (_localized_raster_label(lang,d.get('key'),d['label']) if d.get('key') else d['label'], rgb)
    ...
    used=set(int(v) for v in np.unique(rounded[valid])) if np.any(valid) else set()
    code_info={k:v for k,v in code_info.items() if k in used}
...
def _classify_default_aspect(band, valid_mask, lang='en'):
    import numpy as np
    idx = np.zeros(band.shape, dtype=np.uint8)
    # Düz = -1
    flat = valid_mask & (band < 0)
    idx[flat] = 1
    # 8 yön: N sınıfı hem 0–12.5 hem 327.5–360
    direction_ranges = [
    ...
    return idx, _aspect_code_info(lang)
...
def build_result_image(data, for_export=False):
    ...
    if index in _TOPO_KEYS:
        ...
        if index in ('TOPO', 'TOPO_DEM'):
            ...
            _aspect_class=ee.Image(1).where(result.gte(0).And(result.lt(12.5)),2).where(result.gte(12.5).And(result.lt(57.5)),3).where(result.gte(57.5).And(result.lt(102.5)),4).where(result.gte(102.5).And(result.lt(147.5)),5).where(result.gte(147.5).And(result.lt(192.5)),6).where(result.gte(192.5).And(result.lt(237.5)),7).where(result.gte(237.5).And(result.lt(282.5)),8).where(result.gte(282.5).And(result.lt(327.5)),9).where(result.gte(327.5).And(result.lte(360)),2).where(result.lt(0),1).rename('value').updateMask(result.mask()).clip(roi)
            final_display=_aspect_class
            vis={'min':1,'max':9,'palette':['d9d9d9','3c8d5a','57b894','8ed0c4','c8e1d9','f4b35f','ee7b3a','d83a2e','8f1d14']}

        elif index == 'TOPO_HILLSHADE':
            result = terrain.select('hillshade').rename('value')
            vis = {'min': 0, 'max': 255, 'palette': ['ffffff','d1d5db','6b7280']}
...
def download_geotiff():
    ...
    try:
        ...
        export_image = result if data.get('index') == 'TOPO_ASPECT' else final_display
        ...
        if lulc_index in LULC_CLASS_DEFS:
            try:
                sym_files = _build_lulc_symbology_zip(tif_bytes, lulc_index, safe_name, lang=_request_lang(req_data))
            ...
            if data.get('index') == 'TOPO_ASPECT' and not requested_breaks:
                try:
                    ...
                    _byte, _codes = _classify_default_aspect(_band, _valid, lang=_request_lang(req_data))
                    # Aspect'te 9 gerçek sınıf vardır. Gömülü GeoTIFF ColorMap
                    # 4-bit olduğu için 16 giriş üretir ve ArcMap bu kullanılmayan
                    # girişleri boş/siyah lejant satırı olarak gösterebilir.
                    # LULC'de kullandığımız gibi Aspect'te de ColorMap'i kapat;
                    # gerçek renk+isim bilgisi RAT/VAT/.clr/.legend.csv içinde
                    # yalnızca 9 sınıf olarak taşınsın.
                    sym_files = _build_symbology_files_from_classes(
                        _byte, _profile, _codes, safe_name, embed_colormap=False)
...
def _stamp_exact_band_statistics(tif_bytes, nodata_value=None):
    ...
    try:
        with MemoryFile(tif_bytes) as memfile:
            ...
            # ArcMap/QGIS'in uzaklaştırılmış ölçekte rasterı kaybetmemesi için
            # son gönderilen TIFF'e gerçek overviews/pyramids de yazılır. Bu
            # yalnızca görüntüleme piramididir; ana piksel verisi ve AOI maskesi
            # değiştirilmez. Önceki sürüm yalnızca istatistik yazıyor, piramit
            # üretmiyordu; özellikle TWI/Hillshade gibi float rasterlerde ArcMap
            # 1:250000 gibi ölçeklerde yeniden örnekleme sırasında katmanı siyah
            # veya görünmez gösterebiliyordu.
            try:
            ...
            with out_memfile.open(**profile) as dst:
                ...
                try:
                    from rasterio.enums import Resampling
                    w, h = int(profile.get('width', 0)), int(profile.get('height', 0))
                    levels = [2, 4, 8, 16, 32, 64] if min(w, h) >= 64 else [2, 4, 8, 16]
                    levels = [lv for lv in levels if w // lv >= 32 and h // lv >= 32]
                    if levels:
                        dst.build_overviews(levels, Resampling.nearest)
                        dst.update_tags(ns='rio_overview', resampling='nearest')
                except Exception as _ov_err:
                    print('[SylvaGIS] ⚠️ TIFF overview/pyramid üretilemedi (ana veri korunuyor):', _ov_err)
            try:
                return out_memfile.read()
            finally:
                out_memfile.close()
